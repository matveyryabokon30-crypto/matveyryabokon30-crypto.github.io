...
setup="""async(build)=>{
 ...
 const originalFetch=window.fetch;window.fetch=async(url,opts)=>{if(String(url)!==location.origin+'/functions/v1/pablicus-push')return originalFetch(url,opts);const body=opts.body?JSON.parse(opts.body):null;calls.push({method:opts.method,body});return Response.json(opts.method==='GET'?{publicKey}:{ok:true,installation_id:'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb',tracking:'ready'});};
 window.makeClient=()=>PablicusPush.create({projectUrl:location.origin,apiKey:'SYNTHETIC_PUBLIC',getUserId:()=>currentUser,getSession:async()=>({access_token:'SYNTHETIC_USER_JWT',user:{id:currentUser}})});window.client=makeClient();
