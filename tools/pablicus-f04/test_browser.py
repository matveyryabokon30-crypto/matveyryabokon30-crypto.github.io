...
with sync_playwright() as pw:
 ...
 page.goto(origin+'/f04-probe.html#invite='+token);page.add_script_tag(content=fixture)
 ...
 page.reload();page.add_script_tag(content=fixture);page.evaluate('api.ready()');page.get_by_role('button',name='Принять приглашение',exact=True).wait_for()
 ...
 page.goto(origin+'/f04-probe.html#invite='+token);page.add_script_tag(content=fixture);page.evaluate("mode='unavailable';api.ready()");page.wait_for_function("document.querySelector('[role=status]').textContent.includes('недоступно')")
 ...
 ctx=browser.new_context(service_workers='block');page=ctx.new_page();page.set_default_timeout(25000);errors=[];rpc=[]
