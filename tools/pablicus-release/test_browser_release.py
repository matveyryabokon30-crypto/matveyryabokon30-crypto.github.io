...
import argparse,http.server,json,pathlib,threading,tempfile,time,traceback,urllib.parse
...
a=p.parse_args()
root=pathlib.Path(a.root).resolve();baseline=pathlib.Path(a.baseline).resolve()
...
class Handler(http.server.SimpleHTTPRequestHandler):
 ...
 def do_GET(self):
  self.directory=str(state['current']);path=urllib.parse.urlsplit(self.path).path
  if state['offline']:
   self.send_response(503);self.send_header('Cache-Control','no-store');self.end_headers();return
  if path=='/probe.html':text,mime=probe,'text/html'
  elif path=='/sw.js' and state['corrupt']:
   text=(root/'sw.js').read_text().replace(m['worker_version'],m['worker_version']+'-broken',1);mime='text/javascript'
  ...
  b=text.encode();self.send_response(200);self.send_header('Content-Type',mime);self.send_header('Cache-Control','no-store');self.send_header('Content-Length',str(len(b)));self.end_headers();self.wfile.write(b)
...
with sync_playwright() as pw:
 ...
 ctx=getattr(pw,a.engine).launch_persistent_context(profile.name,headless=True);page=ctx.new_page()
 def version():
 ...
 def cache_state():
  return page.evaluate("""()=>new Promise((resolve,reject)=>{const ch=new MessageChannel();const t=setTimeout(()=>{ch.port1.close();reject(Error('Worker cache diagnostic timeout'))},15000);ch.port1.onmessage=e=>{clearTimeout(t);ch.port1.close();resolve(e.data)};navigator.serviceWorker.controller.postMessage({type:'PABLICUS_RELEASE_CACHE'},[ch.port2]);})""")
 def wait_waiting():
  deadline=time.monotonic()+60
  while time.monotonic()<deadline:
   if page.evaluate('navigator.serviceWorker.getRegistration().then(r=>!!r.waiting)'):return
   page.wait_for_timeout(100)
  raise TimeoutError('Candidate did not enter waiting state')
 try:
  ...
  loads=page.evaluate('Number(sessionStorage.loads)');state['current']=root
  page.evaluate('navigator.serviceWorker.getRegistration().then(r=>r.update())');wait_waiting();page.wait_for_timeout(4000)
  ...
  page.wait_for_function('(n)=>Number(sessionStorage.loads)>n',arg=loads,timeout=30000);page.wait_for_timeout(1500)
  ...
  state['offline']=True
  control=page.evaluate("fetch('network-control-not-precached',{cache:'no-store',signal:AbortSignal.timeout(5000)}).then(r=>r.status)")
  record('network fixture is unavailable during cache proof',control==503)
  offline=page.evaluate('''async(assets)=>{const result=[],items=Object.entries(assets);let cursor=0;await Promise.all(Array.from({length:4},async()=>{while(cursor<items.length){const [name,hash]=items[cursor++];try{const r=await fetch(name,{cache:'no-store',signal:AbortSignal.timeout(5000)});const got=[...new Uint8Array(await crypto.subtle.digest('SHA-256',await r.arrayBuffer()))].map(x=>x.toString(16).padStart(2,'0')).join('');if(!r.ok||got!==hash)result.push({name,status:r.status,hash:got})}catch(e){result.push({name,error:e.name})}}}));return result;}''',m['assets'])
  record('all exact bytes served without a working origin',not offline,{'verified':len(m['assets']),'failed':offline})
  state['offline']=False;state['corrupt']=True
  ...
  errors=[];page.on('pageerror',lambda e:errors.append(str(e)));page.add_init_script('window.initialNativeFetch=window.fetch;')
  page.goto(origin+'/',wait_until='domcontentloaded');page.wait_for_function('window.PablicusDebug && window.PablicusBuild',timeout=45000)
  ...
  checks.append(dict(name='browser completion',passed=False,details=str(exc),traceback=traceback.format_exc()));page.screenshot(path=str(out/(a.engine+'-r0-failure.png')));print(traceback.format_exc())
 finally:
  result=dict(engine=a.engine,build_id=m['build_id'],passed=sum(c['passed'] for c in checks),failed=sum(not c['passed'] for c in checks),checks=checks,boundary='Synthetic persistent browser profile. No physical iPhone or Push delivery acceptance.')
