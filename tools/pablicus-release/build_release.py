...
def build(root, sha):
    ...
    html, count = re.subn(r'(<p\b[^>]*\bid="releaseLabel"[^>]*>)[^<]*(</p>)',
                          lambda m: m.group(1) + 'UI-M2 · ' + sha[:12] + m.group(2), html, count=1)
    if count != 1:
...
(function(g){'use strict';
 ...
 async function inspect(){
  ...
  return {buildId:id,sourceSha,stage:'UI-M2',htmlBuildId:document.querySelector('meta[name="pablicus-release"]')?.content||null,workerBuildId:worker?.buildId||null,workerVersion:worker?.version||null,coherent:worker?.buildId===id,notificationPermission:g.Notification?.permission||'unsupported',standalone:!!(g.matchMedia?.('(display-mode: standalone)').matches||navigator.standalone)};
 }
 g.PablicusBuild=Object.freeze({id,sourceSha,stage:'UI-M2',inspect});
})(window);
...
"""
    ...
    release = dict(schema=1, release='UI-M2', build_id=identity, source_sha=sha,
...
def check(root):
    ...
    if ('UI-M2 · ' + m['source_sha'][:12]) not in html:
