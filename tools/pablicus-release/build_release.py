...
def build(root, sha):
    ...
    html, count = re.subn(r'(<p\b[^>]*\bid="releaseLabel"[^>]*>)[^<]*(</p>)',
                          lambda m: m.group(1) + 'F06 · ' + sha[:12] + m.group(2), html, count=1)
    if count != 1:
...
"""
    ...
    release = dict(schema=1, release='F06', build_id=identity, source_sha=sha,
...
def check(root):
    ...
    if ('F06 · ' + m['source_sha'][:12]) not in html:
