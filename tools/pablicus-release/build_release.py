...
def build(root, sha):
    ...
    html, count = re.subn(r'(<p\b[^>]*\bid="releaseLabel"[^>]*>)[^<]*(</p>)',
                          lambda m: m.group(1) + 'UI-V5 · ' + sha[:12] + m.group(2), html, count=1)
    if count != 1:
...
"""
    ...
    release = dict(schema=1, release='UI-V5', build_id=identity, source_sha=sha,
...
def check(root):
    ...
    if ('UI-V5 · ' + m['source_sha'][:12]) not in html:
