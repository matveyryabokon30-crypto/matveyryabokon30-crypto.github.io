/* Executes the actual Edge handler and SDK against an isolated HTTP fixture.
 * No real project token, account, or private media is used by this test. */
const original=await Deno.readFile('results/fixtures/original.jpg');const checks:any[]=[],store=new Map<string,Uint8Array>();let access=true,downloads=0,rpc=0,originalWrites=0,wrongScope=0,privateBucket=true;
...
const svc=Deno.serve({hostname:'127.0.0.1',port:0,onListen:()=>{}},async req=>{
 const url=new URL(req.url),path=decodeURIComponent(url.pathname),auth=req.headers.get('authorization');
 ...
 if(path==='/storage/v1/bucket/pablicus-media-previews'){return admin?Response.json({id:'pablicus-media-previews',name:'pablicus-media-previews',public:!privateBucket}):Response.json({message:'forbidden'},{status:403});}
 ...
 if(path==='/storage/v1/object/authenticated/message-media/source.jpg'){
...
try{
 ...
 const builtResponse=await call({action:'build',items:[item]}),built=await builtResponse.json();check('real handler builds three private image sizes',builtResponse.ok&&built.kind==='preview'&&store.size===3,{status:builtResponse.status,body:built,stored:store.size});
