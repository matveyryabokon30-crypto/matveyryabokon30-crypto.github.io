...
(function(g){
 ...
 const cards=new Map(),feeds=new Map();let account='',queue=[],timer=0,active=0,feedFlight=null,feedFlightKey='',fallbackUntil=0;
 ...
 function reset(){for(const j of queue)j.reject(abort());queue=[];cards.clear();feeds.clear();feedFlight=null;feedFlightKey='';clearTimeout(timer);timer=0;account=scope(state());}
 ...
 async function flush(){
  ...
  try{
   ...
   counts.batches++;const r=await service().client.rpc('pablicus_home_cards',{p_conversation_ids:jobs.map(j=>j.id)});if(!valid(s))throw abort();
 ...
 function card(id,snapshot){
  ...
  let resolve,reject;const promise=new Promise((ok,no)=>{resolve=ok;reject=no;});const entry={promise,until:Date.now()+30000};cards.set(id,entry);
 ...
 function prime(ids){for(const id of ids.slice(0,48))void card(id).catch(()=>{});}
 ...
 function invalidate(){cards.clear();feeds.clear();}
 g.PablicusHomeData=Object.freeze({card,prime,feed,reset,invalidate,stats:()=>({...counts,queued:queue.length,active}),mark(name){if(!performance.getEntriesByName('pablicus:'+name).length)performance.mark('pablicus:'+name);}});
