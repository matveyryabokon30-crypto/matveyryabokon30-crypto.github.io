...
with sync_playwright() as pw:
 ...
 try:
  ...
  # Playwright's own permissions.spec.ts uses WebKit deterministic mock capture devices.
  # Chromium selects fake hardware by launch flag. Both execute native getUserMedia.
  page.evaluate("""window.asked=0;window.tracks=[];window.errors=[];window.connected={a:false,b:false};window.streams={};window.fixtureGeneration=1;window.peerDiagnostics=[];window.setupStages=['script-ready'];
  const NativePC=window.RTCPeerConnection;
  window.RTCPeerConnection=class extends NativePC{constructor(options){super(options);const diagnostic={connection:'new',ice:'new',signaling:'stable',candidates:0,descriptions:0};peerDiagnostics.push(diagnostic);this.addEventListener('icecandidate',e=>{if(e.candidate)diagnostic.candidates++;});this.addEventListener('iceconnectionstatechange',()=>diagnostic.ice=this.iceConnectionState);this.addEventListener('connectionstatechange',()=>diagnostic.connection=this.connectionState);this.addEventListener('signalingstatechange',()=>diagnostic.signaling=this.signalingState);}};
  const capture=navigator.mediaDevices.getUserMedia.bind(navigator.mediaDevices);
  Object.defineProperty(navigator.mediaDevices,'getUserMedia',{configurable:true,value:async function(options){asked++;setupStages.push('capture-request-'+asked);const stream=await capture(options);tracks.push(...stream.getTracks());setupStages.push('capture-resolved-'+asked);return stream;}});
  const relay=(target,s)=>{if(s.type==='description')setupStages.push('relay-'+s.description.type);setTimeout(()=>void window[target].accept({...s,generation:fixtureGeneration},{video:true}).catch(e=>errors.push(String(e))),0);};
  ...
  document.getElementById('begin').onclick=()=>{setupStages.push('button-gesture');void a.start({video:true}).catch(e=>errors.push(String(e)));};void 0;""")
  page.locator('#begin').click();page.wait_for_function('connected.a&&connected.b',timeout=30000)
  print('RTC diagnostics '+json.dumps(page.evaluate('({asked,errors,connected,peerDiagnostics,setupStages,trackKinds:tracks.map(t=>t.kind)})')),flush=True)
  check('actual paired RTCPeerConnection connects with browser mock-device audio/video',page.evaluate('asked===2&&errors.length===0'))
  ...
  page.evaluate('a.hangup();b.hangup()')
  check('hangup stops every mock-device media track',page.evaluate("tracks.every(t=>t.readyState==='ended')&&!a.active&&!b.active"))
  ...
  print(json.dumps({'engine':a.engine,'checks':checks,'count':len(checks),'result':'PASS','limits':'Loopback native getUserMedia with browser mock capture devices; no production TURN or physical iPhone/background evidence.'},ensure_ascii=False),flush=True)
  if not a.output:
   pathlib.Path('results').mkdir(exist_ok=True);a.output='results/f06-'+a.engine+'.json'
  if a.output:pathlib.Path(a.output).write_text(json.dumps({'engine':a.engine,'checks':checks,'count':len(checks),'result':'PASS','limits':'Loopback native getUserMedia with browser mock capture devices; no production TURN or physical iPhone/background evidence.'},ensure_ascii=False,indent=2))
 except Exception as error:
  diagnostic={}
  try:diagnostic=page.evaluate('({asked:window.asked,errors:window.errors,connected:window.connected,peers:window.peerDiagnostics,setup:window.setupStages})');print('RTC failure diagnostics '+json.dumps(diagnostic),flush=True)
