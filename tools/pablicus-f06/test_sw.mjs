...
const J={kind:'call_invite',notification_id:C,recipient_id:A,sender_id:B,sender_name:'Личное имя',preview:'Входящий звонок',expires_at:new Date(Date.now()+60000).toISOString(),_r1:P._r1};
try{
 let f=fixture();await f.push(J);check('call needs no fabricated conversation',f.shown.length===1&&f.shown[0].title==='Личное имя'&&f.shown[0].body==='Входящий звонок'&&!('conversationId' in f.shown[0].data));
 check('call retains existing R1 observations',f.receipts.map(x=>x.data.state).join(',')==='worker_received,show_resolved');
 check('call data minimal and event tagged',Object.keys(f.shown[0].data).sort().join(',')==='_r1,callId,expiresAt,kind,recipientId'&&f.shown[0].tag==='pablicus-call-'+C);
 await f.push(J);check('duplicate call suppressed',f.shown.length===1&&f.receipts.at(-1).data.state==='duplicate');
 await f.click(f.shown[0].data);const u=new URL(f.opened[0]);check('cold click only event and bound recipient',u.searchParams.get('call')===C&&u.searchParams.get('recipient')===A&&[...u.searchParams.keys()].length===2);
 f=fixture();const sent=[];f.context.self.clients.matchAll=async()=>[{url:'https://app.invalid/',focused:true,focus:async()=>{},postMessage:x=>sent.push(x)}];await f.push(J);await f.click(f.shown[0].data);check('warm click requests authorized event view',sent[0].type==='PABLICUS_CALL_OPEN'&&sent[0].callId===C&&sent[0].recipientId===A&&!('senderId' in sent[0]));
 f=fixture({owner:B});await f.push(J);check('wrong owner suppressed',!f.shown.length&&f.receipts.at(-1).data.state==='suppressed_owner');await f.click({kind:'call_invite',recipientId:A,callId:C});check('wrong owner cannot click through',!f.opened.length);
 for(const expires_at of [undefined,null,'bad',new Date(0).toISOString()]){f=fixture();await f.push({...J,expires_at});check('call expiry mandatory '+String(expires_at),!f.shown.length&&f.receipts.at(-1).data.state==='expired');}
 for(const patch of [{sender_id:'bad'},{recipient_id:'bad'},{notification_id:'bad'}]){f=fixture();await f.push({...J,...patch});check('invalid call identity '+Object.keys(patch)[0],!f.shown.length&&!f.receipts.length);}
 ...
 f=fixture();await f.push({...J,message_id:D,conversation_id:B,task_id:D});check('call cannot become conversation route',!('conversationId' in f.shown[0].data)&&!('taskId' in f.shown[0].data));
 f=fixture();await f.push({...J,media_kind:'video'});check('video call title',f.shown[0].body==='Входящий видеозвонок');
 f=fixture();await f.click({kind:'call_invite',callId:C,recipientId:A,expiresAt:new Date(0).toISOString()});check('expired call click suppressed',!f.opened.length);
}finally{writeFileSync('results/f06-sw-tests.json',JSON.stringify({passed:checks.length,checks,boundary:'Actual SW VM with synthetic transport and storage; no physical device claim.'},null,2));}
