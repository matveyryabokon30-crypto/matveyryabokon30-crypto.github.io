/* P01-R2. Explicit, account-scoped image cache. Never patches window.fetch.
 * Storage authorization is renewed at the original lease duration; cached bytes
 * do not extend a signed URL. Video stays a range-capable network URL.
 */
(function(g){
 ...
 const stats={imageDownloads:0,downloadBytes:0,memoryHits:0,diskHits:0,signRequests:0,coalesced:0,errors:0,peakDownloads:0,evictions:0};
 const aborted=()=>new DOMException('Media scope changed','AbortError');
 function session(){
  const c=g.PablicusController,s=c?.state?.();
  if(!subscribed&&c?.subscribe){subscribed=true;c.subscribe(next=>{if(next.sessionUserId!==owner)changeOwner(next.sessionUserId);});}
  if(s?.sessionUserId!==owner)changeOwner(s?.sessionUserId||null);
 ...
 function changeOwner(id){
  ...
  if(previous)diskWrites=diskWrites.catch(()=>{}).then(()=>purge(previous)).catch(()=>{});
 }
 ...
 function openDB(){
  ...
  if(!dbPromise)dbPromise=new Promise(resolve=>{
   let done=false;const timer=setTimeout(()=>{done=true;resolve(null);},1500);let r;try{r=indexedDB.open('pablicus-images-p01r2',1);}catch{clearTimeout(timer);resolve(null);return;}
   r.onupgradeneeded=()=>{const st=r.result.createObjectStore('images',{keyPath:'key'});st.createIndex('owner','owner');};
   r.onerror=()=>{clearTimeout(timer);dbPromise=null;resolve(null);};
   r.onsuccess=()=>{clearTimeout(timer);if(done){r.result.close();return;}r.result.onversionchange=()=>{r.result.close();dbPromise=null;};resolve(r.result);};
  });return dbPromise;
 }
 async function read(key,s){
  const db=await openDB();assert(s);if(!db)return null;
  return new Promise(resolve=>{try{const t=db.transaction('images'),r=t.objectStore('images').get(key);let v=null;r.onsuccess=()=>v=r.result;t.oncomplete=()=>resolve(valid(s)&&v&&v.until>Date.now()&&v.blob instanceof Blob?v:null);t.onerror=t.onabort=()=>resolve(null);}catch{resolve(null);}});
 }
 async function purge(id){const db=await openDB();if(!db)return;await new Promise(resolve=>{const t=db.transaction('images','readwrite'),r=t.objectStore('images').index('owner').openCursor(IDBKeyRange.only(id));r.onsuccess=()=>{const c=r.result;if(c){c.delete();c.continue();}};t.oncomplete=t.onerror=t.onabort=()=>resolve();});}
 function write(key,blob,s,bucket,expiry){
  diskWrites=diskWrites.catch(()=>{}).then(async()=>{if(!valid(s))return;const db=await openDB();if(!db||!valid(s))return;
   const until=Math.min(Date.now()+24*3600000,expiry||Infinity);
   await new Promise(resolve=>{try{const t=db.transaction('images','readwrite');t.objectStore('images').put({key,owner:s.id,bucket,blob,size:blob.size,touched:Date.now(),until});t.oncomplete=t.onerror=t.onabort=()=>resolve();}catch{resolve();}});
   if(!valid(s))return;
   await new Promise(resolve=>{const t=db.transaction('images','readwrite'),st=t.objectStore('images'),r=st.openCursor(),all=[];let bytes=0;
    r.onsuccess=()=>{const c=r.result;if(c){const v=c.value;if(v.until<=Date.now()){c.delete();}else{bytes+=v.size;all.push({key:v.key,size:v.size,touched:v.touched});}c.continue();return;}
     all.sort((a,b)=>a.touched-b.touched);while(bytes>MAX_DISK&&all.length){const v=all.shift();bytes-=v.size;st.delete(v.key);}};
    t.oncomplete=t.onerror=t.onabort=()=>resolve();});
  }).catch(()=>{});return diskWrites;
 }
 function remember(key,blob,lease,s){
  assert(s);const prior=memo.get(key);if(prior){prior.until=lease;prior.used=Date.now();return prior.url;}
  const entry={url:URL.createObjectURL(blob),bytes:blob.size,until:lease,used:Date.now()};memo.set(key,entry);used+=entry.bytes;
  for(const [id,e] of memo){if(memo.size<=MAX_ENTRIES&&used<=MAX_MEMORY)break;if(id===key)continue;memo.delete(id);used-=e.bytes;URL.revokeObjectURL(e.url);stats.evictions++;}
  return entry.url;
 }
 function hit(key,now=Date.now()){const e=memo.get(key);if(!e||e.until<=now)return null;memo.delete(key);memo.set(key,e);return e;}
 function sign(s,bucket,path,ttl=300){
  ...
  const promise=(async()=>{const r=await s.client.storage.from(bucket).createSignedUrl(path,ttl);assert(s);if(r.error)throw r.error;const u=new URL(r.data?.signedUrl);
   if(u.protocol!=='https:'||u.username||u.password||!['ctcoqgsztdtsazdiwcmd.supabase.co','ctcoqgsztdtsazdiwcmd.storage.supabase.co'].includes(u.hostname))throw Error('Invalid media origin');
   return {url:u.href,until};})();signs.set(key,{until,promise});if(signs.size>256)signs.delete(signs.keys().next().value);
  promise.catch(()=>{if(signs.get(key)?.promise===promise)signs.delete(key);});return promise;
 }
 ...
 async function download(url,s){
  ...
  try{stats.imageDownloads++;const r=await fetch(url,{signal:controller.signal,credentials:'omit',cache:'default'});assert(s);
   if(!r.ok)throw Error('Image HTTP '+r.status);const type=r.headers.get('content-type')||'';
   if(!type.startsWith('image/'))throw Error('Unexpected media type');if(Number(r.headers.get('content-length')||0)>MAX_FILE)throw Error('Image exceeds cache limit');
   const reader=r.body?.getReader();let blob;
   if(reader){const chunks=[];let size=0;for(;;){const part=await reader.read();assert(s);if(part.done)break;size+=part.value.byteLength;if(size>MAX_FILE){await reader.cancel();throw Error('Image exceeds cache limit');}chunks.push(part.value);}blob=new Blob(chunks,{type});}
   else blob=await r.blob();assert(s);if(!blob.size||blob.size>MAX_FILE)throw Error('Invalid image size');stats.downloadBytes+=blob.size;return blob;
 ...
 async function preview(blob,width,s){
  ...
  try{
   if(g.createImageBitmap)try{bitmap=await createImageBitmap(blob);}catch{}
   if(!bitmap){url=URL.createObjectURL(blob);img=new Image();img.loading='eager';img.src=url;await img.decode();}
   assert(s);const source=bitmap||img,w=source.width||source.naturalWidth,h=source.height||source.naturalHeight;
   if(w<=width&&h<=width)return blob;
   const scale=Math.min(1,width/Math.max(w,h)),canvas=document.createElement('canvas');canvas.width=Math.max(1,Math.round(w*scale));canvas.height=Math.max(1,Math.round(h*scale));const ctx=canvas.getContext('2d');ctx.drawImage(source,0,0,canvas.width,canvas.height);
 ...
 function resolve(bucket,path,options={}){
  ...
  const ttl=Math.min(300,Math.max(30,options.ttl||300)),expiry=Number(options.expiresAt)||Infinity;
  if(expiry<=Date.now())return Promise.reject(Error('Media expired'));
  ...
  if(cached&&cached.until<=expiry){stats.memoryHits++;return Promise.resolve(cached.url);}
  ...
  const job=(async()=>{
   try{const lease=await sign(s,bucket,path,ttl);assert(s);const until=Math.min(lease.until,expiry),mem=memo.get(key);if(mem){mem.until=until;stats.memoryHits++;return mem.url;}
    ...
    if(!base){base=(async()=>{const disk=await read(originalKey,s);if(disk){stats.diskHits++;return disk.blob;}return schedule(s,async()=>{const b=await download(lease.url,s);void write(originalKey,b,s,bucket,expiry);return b;},options.priority||0);})();pending.set(baseKey,base);base.finally(()=>{if(pending.get(baseKey)===base)pending.delete(baseKey);}).catch(()=>{});}else stats.coalesced++;
