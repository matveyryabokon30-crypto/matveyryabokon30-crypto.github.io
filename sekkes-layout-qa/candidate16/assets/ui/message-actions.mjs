...
export function messageActions({doc=document,signal,account,notify,compose,currentScope,ownerAllowed}){
 const entries=new WeakMap(),selected=new Set();let user='',saved={hidden:[],pins:{},reactions:{}},timer=0,start=null,menu=null,suppressClick=false;
 ...
 function panel(label){close();const overlay=el('div','message-menu-overlay'),box=el('div','message-menu');box.setAttribute('role','dialog');box.setAttribute('aria-label',label);const heading=el('p','message-menu-title',label);box.append(heading);overlay.append(box);overlay.addEventListener('click',e=>{if(e.target===overlay)close()});doc.body.append(overlay);menu=overlay;return box;}
 ...
 function open(node){load();const entry=entries.get(node);if(!entry)return;const box=panel('Действия с сообщением');const reactions=el('div','message-reactions');for(const emoji of ['❤️','👍','🔥','😁','🙏']){const b=button(emoji,()=>{saved.reactions[entry.key]=saved.reactions[entry.key]===emoji?'':emoji;save();apply(node);close()});b.setAttribute('aria-label','Реакция '+emoji);reactions.append(b)}box.append(reactions);
  box.append(button('Ответить',()=>{close();compose(entry.scope,entry.text.split('\n').map(line=>'> '+line).join('\n')+'\n\n')}),button('Копировать',()=>copy([node])),button(saved.pins[entry.key]?'Открепить':'Закрепить',()=>{if(saved.pins[entry.key])delete saved.pins[entry.key];else saved.pins[entry.key]={scope:entry.scope,text:entry.text.slice(0,300)};save();apply(node);refreshPins();close()}),button('Переслать',()=>forward([node])),button('Удалить',()=>remove([node])),button('Выбрать',()=>{close();toggle(node)}),button('Выделить текст',()=>{close();const range=doc.createRange();range.selectNodeContents(node.querySelector('.rich-message')||node);const selection=doc.getSelection();selection.removeAllRanges();selection.addRange(range)}),button('Отмена',close));focusPanel(box);
 }
 const find=e=>e.target.closest?.('[data-action-message]');
 function cancel(){clearTimeout(timer);timer=0;start=null;}
 doc.addEventListener('pointerdown',e=>{cancel();const node=find(e);if(!node||e.button!==0||e.target.closest('a,button,audio,video,input,textarea,summary'))return;start={x:e.clientX,y:e.clientY,node};timer=setTimeout(()=>{timer=0;suppressClick=true;open(node)},500)},{passive:true,signal});
 ...
 doc.addEventListener('click',e=>{const node=find(e);if(!node)return;if(suppressClick){suppressClick=false;e.preventDefault();return}if(selected.size&&!e.target.closest('a,button')){e.preventDefault();toggle(node)}},{signal});
