...
(function(g){
 ...
 const life=new AbortController(),cache=new Map();let account=key(),ui=null,frame=0,dataFrame=0,lastScroll=0,linkHandled=false;
 ...
 async function media(story,retry=false){
  ...
  const k=key(),ck=k+':media:'+story.id,hit=cache.get(ck);if(!retry&&hit?.until>Date.now())return hit.promise;
  const promise=(async()=>{const r=await S().client.storage.from('pablicus-story-media').createSignedUrl(path,120);if(!valid(k))throw Error('Session changed');if(r.error)throw r.error;const u=new URL(r.data.signedUrl);if(u.protocol!=='https:'||u.username||u.password)throw Error('Invalid URL');return u.href;})();
  cache.set(ck,{until:Date.now()+90000,promise});if(cache.size>80)cache.delete(cache.keys().next().value);try{return await promise;}catch(e){cache.delete(ck);throw e;}
 }
 ...
 function measure(){if(!ui)return;const pad=parseFloat(getComputedStyle(ui.ws).paddingBottom)||0;ui.ws.style.setProperty('--story-min-list',Math.max(0,ui.ws.clientHeight-G.compact-pad)+'px');paint();}
 function paint(){
  ...
  Array.from(nodes.values()).forEach((n,i)=>{const origin=16+i*G.step-scrollX,target=start+(i-first)*25;const x=(target-origin)*p;
 ...
 function mount(ws){
  ...
  const ro=new ResizeObserver(measure);ui.ro=ro;ro.observe(ws);render();measure();
 }
 function unmount(){if(!ui)return;lastScroll=ui.ws.scrollTop;ui.abort.abort();ui.ro.disconnect();ui.anchor.remove();ui.spacer.remove();delete ui.ws.dataset.storyShelf;ui.ws.style.removeProperty('--story-min-list');ui=null;cancelAnimationFrame(frame);frame=0;}
