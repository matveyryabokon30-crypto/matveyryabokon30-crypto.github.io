/* Pablicus 0.1.0-rc5. Existing Auth, server sequence, RPCs, Realtime + polling.
...
(() => {'use strict';
 ...
 const signed=new Map(),workspaceUploadState=new Map(),cacheKey=()=>`pablicus:${user?.id}:dialogs`,focusKey=()=>`pablicus:${user?.id}:focus`,messageCacheKey=id=>`pablicus:${user?.id}:messages:${id}`;
 let chatListGestures=null,chatListOwner=null,chatArchive=false,chatPrefsRevision=0,chatListRenderPending=false;
 let dialogsRequest=null,dialogsLast=0,dialogsRealtimeTimer=0;
 const chatPrefs=new Map(),chatActionBusy=new Set();
 const replyCache=new Map();
 const mediaViewer=PablicusMediaViewer.create({resolveUrl:mediaUrl,download:downloadAttachment});
 const profilePage=PablicusProfilePage.create({client:sb,getUser:()=>user,getProfile:()=>profile,settingsButton:$('profileSettings'),backButton:$('profileBack'),mountSettings:renderProfileSettings,dialog,shareProfile,onError:problem,toast,onProfileChanged:p=>{profile=p;safeSet('pablicus:'+user.id+':profile',p)}});
 $('profileSettings').append(PablicusIcons.icon('settings'));$('profileBack').append(PablicusIcons.icon('back'));
 ...
 const people=PablicusPeople.create({search:async query=>{const uid=user?.id;if(!uid)throw Error('Войдите в приложение');const r=await sb.rpc('pablicus_search_people',{p_query:query});if(user?.id!==uid)return[];if(r.error)throw r.error;return r.data||[]},onOpen:openPerson,onError:problem,onDiscover:()=>contactDiscovery.open()});
 const contactDiscovery=PablicusContactDiscovery.create({client:sb,getUser:()=>user,onOpen:openPerson,onSearch:()=>people.open(),onShare:()=>shareProfile()});
 const joinedNotices=PablicusJoined.create({client:sb,getUser:()=>profile?.is_approved?user:null,onOpen:openPerson});
 const callPanel=el('dialog','pablicusCall'),callTitle=el('h2','','Звонок'),callState=el('p',''),callLocal=el('video','callLocal'),callRemote=el('video','callRemote'),callActions=el('div','callActions');callPanel.setAttribute('aria-label','Звонок');callState.setAttribute('role','status');for(const v of [callLocal,callRemote]){v.autoplay=true;v.playsInline=true;}callLocal.muted=true;callPanel.append(callTitle,callState,callRemote,callLocal,callActions);document.body.append(callPanel);
 const callButton=el('button','plain');const callIcon=document.createElementNS('http://www.w3.org/2000/svg','svg');callIcon.setAttribute('viewBox','0 0 24 24');callIcon.setAttribute('width','22');callIcon.setAttribute('height','22');callIcon.setAttribute('fill','none');callIcon.setAttribute('stroke','currentColor');callIcon.setAttribute('stroke-width','1.5');callIcon.setAttribute('aria-hidden','true');const callPath=document.createElementNS('http://www.w3.org/2000/svg','path');callPath.setAttribute('d','M6 3h3l2 5-2 2c1 3 2 4 5 5l2-2 5 2v3c0 2-2 3-4 2C10 19 5 14 3 7 2 5 4 3 6 3Z');callIcon.append(callPath);callButton.append(callIcon);callButton.type='button';callButton.setAttribute('aria-label','Позвонить');callButton.hidden=true;document.querySelector('.chatHeaderActions').prepend(callButton);
 function callAction(text,fn){const b=el('button','',text);b.type='button';b.onclick=fn;callActions.append(b);return b;}
 function renderCall(s){callButton.hidden=!s.available;callButton.disabled=!!s.call;callActions.replaceChildren();callState.textContent=({incoming:s.call?.media==='video'?'Входящий видеозвонок':'Входящий голосовой звонок',ringing:'Вызов…',accepting:'Соединяем…',connecting:'Соединяем…',connected:'Соединение установлено',reconnecting:'Восстанавливаем соединение…',checking:'Проверяем доступность…',unavailable:'Звонки пока недоступны',timeout:'Время ожидания истекло','connection-failed':'Не удалось восстановить соединение','other-device':'Звонок принят на другом устройстве',failed:'Не удалось начать звонок',ended:'Звонок завершён'})[s.phase]||'';if(s.call||s.phase==='checking'){callTitle.textContent=dialogs.find(d=>d.id===s.call?.conversation_id)?.title||(s.call?.media==='video'?'Видеозвонок':'Звонок');if(!callPanel.open)callPanel.showModal();if(s.phase==='incoming')callAction('Принять',()=>void calls.accept());if(s.phase==='connected'){callAction(s.muted?'Включить микрофон':'Выключить микрофон',()=>calls.setMuted(!s.muted));if(s.call.media==='video')callAction(s.video?'Выключить камеру':'Включить камеру',()=>calls.setVideo(!s.video));callAction('Включить звук',()=>void callRemote.play().catch(()=>{}));}callAction(s.phase==='incoming'?'Отклонить':'Завершить',()=>void calls.end());}else if(callPanel.open){callAction('Закрыть',()=>callPanel.close());} }
 const calls=PablicusCallClient.create({client:sb,getUser:()=>profile?.is_approved?user:null,onChange:renderCall,onLocalStream:stream=>{callLocal.srcObject=stream;callLocal.hidden=!stream;},onRemoteStream:stream=>{callRemote.srcObject=stream;callRemote.hidden=!stream;if(stream)void callRemote.play().catch(()=>{});}});
 window.PablicusCallsActive=()=>{const s=calls.snapshot();return !!s.call||s.phase==='checking';};
 callPanel.addEventListener('cancel',e=>{e.preventDefault();void calls.end();callPanel.close();});callButton.onclick=()=>{if(!current||!calls.snapshot().available)return;callTitle.textContent=current.title||'Звонок';callState.textContent='Выберите звонок';callActions.replaceChildren();const cid=current.id;callAction('Голосовой звонок',()=>void calls.start(cid,'audio'));callAction('Видеозвонок',()=>void calls.start(cid,'video'));callAction('Закрыть',()=>callPanel.close());callPanel.showModal();};
 const personalInvites=PablicusPersonalInvites.create({client:sb,getUser:()=>profile?.is_approved?user:null,onOpen:openPerson,onGenericShare:shareApp});
 const libraryContext=()=>({userId:user?.id,conversationId:current?.id,title:current?.title||'Разговор',epoch});
 const libraryCurrent=c=>!!c?.userId&&user?.id===c.userId&&current?.id===c.conversationId&&epoch===c.epoch;
 async function libraryPage({context,query='',cursor=null,limit=36,signal},kind=null){
  if(!libraryCurrent(context))throw Error('Разговор изменился');
  const size=Math.min(60,Math.max(1,limit)),params={p_conversation_id:context.conversationId,p_query:query,p_before_seq:typeof cursor==='object'?cursor?.server_seq??null:cursor,p_limit:size+1};
  if(kind){params.p_kind=kind;params.p_before_index=cursor?.block_index??null;}
  let request=sb.rpc(kind?'pablicus_chat_materials':'pablicus_search_messages',params);if(signal&&request.abortSignal)request=request.abortSignal(signal);
  const r=await request;if(!libraryCurrent(context)||signal?.aborted)throw new DOMException('Запрос отменён','AbortError');if(r.error)throw r.error;
  const all=r.data||[],items=all.slice(0,size),last=items.at(-1);return{items,total:null,nextCursor:all.length>size?(kind?{server_seq:last.server_seq,block_index:last.block_index}:last.server_seq):null};
 }
 async function libraryMessage(id,context=libraryContext()){
  if(!libraryCurrent(context))throw Error('Разговор изменился');
  const r=await sb.from('messages').select('*').eq('conversation_id',context.conversationId).eq('id',id).maybeSingle();
  if(!libraryCurrent(context))throw Error('Разговор изменился');if(r.error)throw r.error;if(!r.data||r.data.deleted_at)throw Error('Сообщение удалено или больше недоступно');return r.data;
 }
 const chatLibrary=PablicusChatLibrary.create({getContext:libraryContext,searchMessages:args=>libraryPage(args),listMaterials:args=>libraryPage(args,args.tab),getMessage:libraryMessage,onLocate:async id=>locateMessage(await libraryMessage(id)),resolveUrl:mediaUrl,onOpenMedia:(items,index)=>mediaViewer.open(items,index),onDownload:downloadAttachment,onError:problem});
 $('chatTitle').onclick=()=>chatLibrary.open({tab:'media'});$('chatLibraryOpen').onclick=()=>chatLibrary.open({tab:'search'});
 let canvasVisible=false,canvasSwitch=0,canvasEpoch=0;
 const canvasContext=()=>({userId:user?.id,conversationId:current?.id,title:current?.title||'Разговор',epoch:canvasEpoch});
 const canvasCurrent=c=>!!c?.userId&&user?.id===c.userId&&current?.id===c.conversationId&&canvasEpoch===c.epoch;
 ...
 function renderCanvasContent({host,content,context,signal}){
  ...
  const view=PablicusRichMessage.render(content,{mountRoot:host,isActive:active,linkify:true,naturalMedia:true,
   resolveUrl:async(path,block)=>{if(!active())throw new DOMException('Разговор изменился','AbortError');const url=await signedUrl(path||block?.path);if(!active())throw new DOMException('Разговор изменился','AbortError');return url;},
 ...
 function appNavigationAllowed({target,params={}}){
  if(['conversation','canvas'].includes(target.screen)&&(!user||!target.conversationId||opening))return false;
  if(!canvasVisible)return true;
  if(target.screen==='canvas'&&target.conversationId===current?.id&&!params.sourceMessage&&!params.canvasOptions?.taskId)return true;
  return canvasNavigationAllowed();
 }
 function paintConversationView(){
  window.PablicusShell.conversationMode(canvasVisible);
 }
 ...
 function showConversationView(){
  if(window.PablicusController)return window.PablicusController.navigate({section:'chats',screen:'conversation',conversationId:current?.id});
  if(!canvasNavigationAllowed())return Promise.resolve(false);
  return mountConversationView();
 }
 async function mountConversationView(){++canvasSwitch;chatCanvas.close();canvasVisible=false;paintConversationView();requestAnimationFrame(()=>PablicusChat.list?.refreshFont());return true;}
 function showCanvasView(sourceMessage,options={}){
  const target={section:'chats',screen:'canvas',conversationId:current?.id};
  const params={sourceMessage,canvasOptions:options};
  if(window.PablicusController)return window.PablicusController.navigate(target,params);
  if(!appNavigationAllowed({target,params}))return Promise.resolve(false);
  return mountCanvasView(sourceMessage,options);
 }
 async function mountCanvasView(sourceMessage,options={},isCurrent=()=>true){
  if(!isCurrent()||!user||!current||opening)return;const context=canvasContext(),ticket=++canvasSwitch;
  if(canvasVisible&&!sourceMessage&&!options.taskId&&chatCanvas.element?.dataset.state==='ready')return;
  PablicusChat.collapseEditor();
  await PablicusChat.persistDraft();if(!isCurrent()||ticket!==canvasSwitch||!canvasCurrent(context))return;
 ...
 async function taskFromMessage(message,blockId){
  if(!current||message.conversation_id!==current.id)return;
  const block=blockId?message.attachment_metadata?.blocks?.find(b=>b.id===blockId):null;
  const text=block?(block.type==='text'?block.text:PablicusRichMessage.attachmentLabel(block)):(message.body||'');
 ...
 const tasksHome=PablicusTasksHome.create({getContext:tasksContext,
  load:async({context,view,query,today,cursor,signal})=>{
   const data=await tasksRequest('pablicus_list_tasks_v2',{p_view:view,p_query:query,p_today:today,p_cursor:cursor,p_limit:40,p_timezone:localTimezone()},context,signal);
  ...
  onToggle:async({context,task,completed,signal})=>{
   const data=await tasksRequest('pablicus_update_canvas_task_v2',{p_conversation_id:task.conversation_id,p_task_id:task.id,p_expected_revision:task.revision,p_title:task.title,p_assignee_id:task.assignee_id,p_due_date:task.due_date,p_completed:completed,p_schedule:null,p_archived:null},context,signal);
   ...
   workspaceQuick.refresh();return data;
  },
  onOpen:async({context,task})=>{
   if(!tasksCurrent(context)||opening)return;
   await openWorkspaceItem(task,true);
  }
 });
 ...
 async function workspaceRequest(name,params,signal){
  const uid=user?.id,token=authVersion;if(!uid)throw new DOMException('Войдите в приложение','AbortError');
  let request=sb.rpc(name,params);if(signal&&request.abortSignal)request=request.abortSignal(signal);
  const result=await request;if(user?.id!==uid||token!==authVersion||signal?.aborted)throw new DOMException('Аккаунт изменился','AbortError');
  if(result.error)throw result.error;return result.data;
 }
 const workspaceQuick=PablicusWorkspaceQuick.create({host:$('workspaceQuick'),getContext:()=>({userId:user?.id,epoch:authVersion,active:!!user&&!!current&&!$('app').hidden}),
  loadTasks:({today,timezone,cursor,limit,signal})=>workspaceRequest('pablicus_list_tasks_v2',{p_view:'today',p_query:'',p_today:today,p_timezone:timezone,p_cursor:cursor,p_limit:limit},signal),
  loadProjects:({cursor,limit,signal})=>workspaceRequest('pablicus_list_projects',{p_query:'',p_cursor:cursor,p_limit:limit},signal),
  onTask:task=>openWorkspaceItem(task,true),onProject:project=>openWorkspaceItem(project,false),onError:problem});
 async function openWorkspaceItem(item,isTask,review=false){
  const uid=user?.id,target=item?.conversation_id;if(!uid||opening||!validUuid(target)||(isTask&&!validUuid(item.id)))return;
  if(current?.id!==target){await PablicusChat.flush();if(user?.id!==uid)return;if(await openConversation(dialogs.find(d=>d.id===target)||{id:target,title:item.conversation_title||'Разговор'},{canvas:true})===false)return;}
  if(user?.id!==uid||current?.id!==target)return;
  await showCanvasView(null,isTask?{taskId:item.id,taskReview:review}:{});
 }
 ...
 async function openPushConversation(id,recipientId,task=null){const uid=user?.id;if(!uid||uid!==recipientId||!validUuid(id))return;if(opening)throw Error('Дождитесь открытия разговора');const notice=pushTask(task);if(!notice)await PablicusChat.flush();if(user?.id!==uid)return;await loadDialogs();if(user?.id!==uid)return;const target=dialogs.find(d=>d.id===id);if(!target)throw Error('Этот разговор больше недоступен');if(notice){await openWorkspaceItem({id:notice.taskId,conversation_id:id,conversation_title:target.title},true,notice.kind==='task_followup');return;}if(current?.id!==id)await openConversation(target);}
 ...
 function deliverInbox(message,uid,token){
  ...
  if(dialogs.find(d=>d.id===message.conversation_id)?.muted)return;
 ...
 function startInbox(){stopInbox();if(!user)return;const uid=user.id,token=inboxEpoch;for(const d of dialogs)inboxSeq.set(d.id,+d.last_seq||0);inboxContext();
  inboxChannel=sb.channel('pablicus-inbox-'+uid).on('postgres_changes',{event:'INSERT',schema:'public',table:'messages'},payload=>{deliverInbox(payload.new,uid,token);if(user?.id===uid&&token===inboxEpoch)refreshDialogsSoon();}).subscribe(state=>{if(state==='SUBSCRIBED')pollInbox()});
 }
 ...
 function clearSessionView(){calls.clear();joinedNotices.clear({preservePending:!user});personalInvites.clear({preserveUnbound:!user});if(sessionViewCleared)return;dialogsRequest=null;dialogsLast=0;clearTimeout(dialogsRealtimeTimer);dialogsRealtimeTimer=0;window.PablicusHomeData?.reset();profilePage.destroy();disposeChatList();chatPrefs.clear();chatActionBusy.clear();chatArchive=false;chatPrefsRevision++;sessionViewCleared=true;opening=false;workspaceQuick.reset();resetTasksHome();resetConversationView();const pushCleanup=pushNotifications.clear();mediaViewer.close();chatLibrary.reset();people.close();contactDiscovery.clear();messageTools.clear();stopInbox();PablicusRichMessage.stopAll?.();replyCache.clear();sessionCleanup=sessionCleanup.then(()=>Promise.all([pushCleanup,window.PablicusChat?.leave()])).catch(e=>toast('Не удалось сохранить черновик на устройстве. '+e.message));if(passkeys?.snapshot().busy&&passkeys.snapshot().operation!=='signIn')passkeys.cancel();$('productDialog').close();$('dialogContent').replaceChildren();user=null;profile=null;dialogs=[];rows=[];current=null;epoch++;signed.clear();window.PablicusMediaCache?.clear();if(channel)sb.removeChannel(channel);channel=null;window.PablicusShell.show('home');window.PablicusShell.authentication(false);}
 async function authenticate(session,{signal,verifiedPasskey=false}={}){
  ...
  if(user?.id===session.user.id&&profile){void pushNotifications.sessionRestored(session);calls.ready(session);await window.PablicusController?.sessionChanged(session.user.id);return}
  ...
  renderHome();window.PablicusAvatarStoriesUI?.flushHome();window.PablicusStoriesCore?.flush();window.PablicusShell.authentication(true);window.PablicusShell.project(window.PablicusController.state());$('home').classList.remove('auth-booting');window.PablicusHomeData?.mark('home-visible');void pushNotifications.sessionRestored(session);calls.ready(session);await loadDialogs();if(attempt!==authVersion||user?.id!==uid)return;startInbox();pump();showPersonLink();showPushConversation();void personalInvites.ready();void joinedNotices.ready();
 }
 ...
 passwordLogin=PablicusLogin.mount({client:sb,recoveryClient,projectUrl:URL,canSignIn:()=>!passkeySigninActive&&!publicKeyFlow.snapshot().busy,authenticate:session=>{trustExplicitSignIn();return authenticate(session)}});
 sb.auth.onAuthStateChange((_event,session)=>{
  if(_event==='TOKEN_REFRESHED')calls.ready(session);
  if(_event==='SIGNED_OUT'){calls.clear();personalInvites.clear();joinedNotices.clear();}
  if(passkeySigninActive){
   const previous=passkeyAuthEvent?.session?.user.id;
   passkeyAuthEvent={event:_event,session};
   if(!session||(previous&&previous!==session.user.id)){++authVersion;++authEventSerial;passkeys.cancel();void window.PablicusController?.sessionChanged(null).catch(problem);clearSessionView()}
   return;
  }
  const observed=++authEventSerial;
  if(authBooting){pendingAuthEvent={session};return}
  const blocked=passkeyUnvalidated||safeGet(passkeyGuardKey)===true;
  const identity=blocked?null:session?.user.id||null;
  const identityChanged=identity!==(window.PablicusController?.state().sessionUserId||null);
  if(identityChanged||identity!==(user?.id||null))++authVersion;
  // Use the event identity, not getSession inside the SDK callback/lock.
  void window.PablicusController?.sessionChanged(identity).catch(problem);
  if(identityChanged&&user)clearSessionView();
  setTimeout(()=>{
   if(observed!==authEventSerial)return;
   if(session?.user.id===user?.id&&profile&&!blocked){void pushNotifications.sessionRestored(session);calls.ready(session);return;}
   authenticate(blocked?null:session).catch(e=>{if(observed===authEventSerial)$('loginError').textContent=e.message});
  },0);
 });
 const authRestoreEvent=authEventSerial;
 PablicusOAuthSession.restore({client:sb,storageKey:authStorageKey,href:authCallbackHref}).then(async session=>{
  if(session&&new window.URL(authCallbackHref).searchParams.has('code'))trustExplicitSignIn();
  authBooting=false;const observed=authEventSerial,next=pendingAuthEvent?pendingAuthEvent.session:session;pendingAuthEvent=null;
  try{await authenticate(next)}catch(error){if(observed===authEventSerial)$('loginError').textContent=error.message}
 },async error=>{
  authBooting=false;const observed=authEventSerial,pending=pendingAuthEvent;pendingAuthEvent=null;
  if(pending){try{await authenticate(pending.session)}catch(e){if(observed===authEventSerial)$('loginError').textContent=e.message}return}
  if(observed!==authRestoreEvent)return;
  await authenticate(null);if(observed===authEventSerial)$('loginError').textContent=error.message;
 }).finally(()=>{$('home').classList.remove('auth-booting')}).catch(problem);
 function fetchDialogSnapshot(){
 ...
 function storageSize(bytes){const value=Number(bytes);if(!Number.isFinite(value)||value<0)return'—';if(value===0)return'0 Б';const units=['Б','КБ','МБ','ГБ','ТБ'],index=Math.min(4,Math.floor(Math.log(value)/Math.log(1024)));return new Intl.NumberFormat('ru',{maximumFractionDigits:index?1:0}).format(value/1024**index)+' '+units[index];}
 function mountStorageUsage(parent){
  const uid=user?.id,box=el('section','storageUsage');box.id='storageUsage';box.setAttribute('aria-label','Хранилище вложений');
  const heading=el('h2','','Хранилище'),total=el('strong','','—'),own=el('strong','','—'),status=el('p','muted','Проверяем занятое место…'),refresh=el('button','setting','Обновить данные');
  total.id='storageTotal';own.id='storageOwn';status.id='storageStatus';status.setAttribute('role','status');refresh.type='button';refresh.id='storageRefresh';
  const totalRow=el('div','storageUsageRow'),ownRow=el('div','storageUsageRow');totalRow.append(el('span','','Все вложения Pablicus'),total);ownRow.append(el('span','','Ваши загрузки'),own);
  box.append(heading,totalRow,ownRow,status,refresh);parent.append(box);let request=0;
  const active=ticket=>user?.id===uid&&page==='profile'&&!current&&box.isConnected&&ticket===request;
  async function measure(){const ticket=++request;refresh.disabled=true;status.textContent='Проверяем занятое место…';try{
   const r=await sb.rpc('pablicus_storage_usage');if(!active(ticket))return;if(r.error)throw r.error;const data=Array.isArray(r.data)?r.data[0]:r.data;
   if(!data||[data.total_bytes,data.own_bytes].some(v=>v===null||v===''||!Number.isFinite(Number(v))||Number(v)<0))throw Error('Размер недоступен');
   total.textContent=storageSize(data.total_bytes);own.textContent=storageSize(data.own_bytes);box.dataset.state='ready';
   status.textContent=Number(data.unknown_size_count)>0?'Размер части файлов пока неизвестен. Показан подтверждённый объём.':'Объём файлов в облаке, включая загрузки, ещё не отправленные в чат. Удаление сообщения само по себе не освобождает место.';
  }catch(e){if(active(ticket)){total.textContent='—';own.textContent='—';box.dataset.state='unavailable';status.textContent='Не удалось узнать занятое место. Повторите проверку.';}}finally{if(active(ticket))refresh.disabled=false;}}
  refresh.onclick=measure;measure();
 }
 function goConversation(d,options={}){const target={section:'chats',screen:options.canvas?'canvas':'conversation',conversationId:d.id,resourceId:null};if(window.PablicusController)return window.PablicusController.navigate(target);if(!appNavigationAllowed({target}))return Promise.resolve(false);return mountConversation(d,options)}
 function withChatPreference(d){const pref=chatPrefs.get(d.id)||{};return {...d,pinned:!!pref.pinned,muted:!!pref.muted,archived:!!pref.archived,marked_unread:!!pref.marked_unread,hidden_until_seq:pref.hidden_until_seq??null};}
 function chatHidden(d){return d.hidden_until_seq!==null&&d.hidden_until_seq!==undefined&&Number(d.last_seq)<=Number(d.hidden_until_seq);}
 function disposeChatList(){
  chatListGestures?.destroy();chatListGestures=null;chatListOwner=null;chatListRenderPending=false;
  const panel=$('chatUpperPanel');if(panel){$('chatFilters').before($('searchChats'));panel.remove();$('chatUpperToggle')?.remove();}
 }
 function ensureChatList(){
  if(chatListOwner!==user.id){disposeChatList();chatListOwner=user.id;const uid=user.id,top=$('workspace').querySelector('.workspaceTop');
   const toggle=el('button','chatUpperToggle','Поиск и действия');toggle.id='chatUpperToggle';toggle.setAttribute('aria-controls','chatUpperPanel');
   const panel=el('section','chatUpperPanel');panel.id='chatUpperPanel';panel.setAttribute('aria-label','Поиск и быстрые действия');
   const body=el('div','chatUpperContent'),quick=el('div','chatQuickActions');body.append($('searchChats'),quick);panel.append(body);top.prepend(toggle,panel);
   const find=el('button','chatQuickAction','Найти человека');find.id='chatFindPeople';find.prepend(PablicusMessageMenu.icon('users'));find.onclick=()=>people.open($('searchChats').value);
   const saved=el('button','chatQuickAction','Избранное');saved.id='chatOpenSaved';saved.prepend(PablicusMessageMenu.icon('saved'));saved.onclick=async()=>{saved.disabled=true;try{const r=await sb.rpc('start_saved_conversation');if(r.error)throw r.error;if(user?.id!==uid)return;await loadDialogs();if(user?.id===uid)await openConversation(dialogs.find(d=>d.id===r.data)||{id:r.data,title:'Избранное'});}catch(e){if(user?.id===uid)problem(e);}finally{saved.disabled=false;}};
   const archive=el('button','chatQuickAction','Архив чатов');archive.id='chatArchiveToggle';archive.prepend(PablicusMessageMenu.icon('archive'));archive.onclick=()=>{chatArchive=!chatArchive;renderHome();};quick.append(find,saved,archive);
   chatListGestures=PablicusChatListView.gestures($('workspace'),{panel,toggle,isActive:()=>user?.id===uid&&!current&&page==='chats'&&!$('home').hidden&&PablicusController.state().screen==='home',onIdle:()=>{if(chatListRenderPending){chatListRenderPending=false;renderHome();}}});
  }
  $('chatArchiveToggle').setAttribute('aria-pressed',String(chatArchive));
 }
 async function changeChatPreference(d,action,value=null){
  const uid=user?.id,key=uid+':'+d.id;if(!uid||chatActionBusy.has(key))return;
  chatActionBusy.add(key);chatListGestures?.close();chatPrefsRevision++;const row=[...document.querySelectorAll('.chatCard')].find(n=>n.dataset.conversationId===d.id);row?.querySelectorAll('button').forEach(b=>b.disabled=true);
  try{const r=await sb.rpc('pablicus_chat_preferences',{p_action:action,p_conversation_id:d.id,p_value:value});if(r.error)throw r.error;if(user?.id!==uid)return;
   chatPrefs.set(d.id,r.data);dialogs=dialogs.map(item=>{if(item.id!==d.id)return item;return withChatPreference(action==='read'?{...item,unread_count:0,last_read_seq:item.last_seq}:item);});safeSet(cacheKey(),dialogs);if(!current&&page==='chats')renderHome();return true;
  }catch(e){if(user?.id===uid)problem(e);}finally{chatPrefsRevision++;chatActionBusy.delete(key);row?.querySelectorAll('button').forEach(b=>b.disabled=false);}
 }
 function confirmChatDelete(d){
  chatListGestures.close();const c=dialog('Удалить чат из списка?'),uid=user.id;
  c.append(el('p','',d.title||'Разговор'),el('p','muted','Чат исчезнет только из вашего списка. Сообщения у участников останутся. Новое сообщение вернёт чат; его также можно открыть через поиск человека.'));
  const cancel=el('button','','Отмена'),remove=el('button','danger','Удалить из списка');remove.id='confirmChatDelete';cancel.onclick=()=>$('productDialog').close();remove.onclick=async()=>{if(user?.id!==uid)return;remove.disabled=true;const done=await changeChatPreference(d,'delete');if(done&&user?.id===uid)$('productDialog').close();else remove.disabled=false;};c.append(cancel,remove);cancel.focus();
 }
 function decorateChatRow(row,d,button,focus){
  const face=el('div','chatSwipeFace');face.append(button,focus);const before=el('div','chatSwipeActions before'),after=el('div','chatSwipeActions after');
  before.setAttribute('aria-label','Прочтение и закрепление');after.setAttribute('aria-label','Звук, удаление и архив');
  const action=(host,key,label,icon,run)=>{const b=el('button','chatSwipeAction',label);b.dataset.chatAction=key;b.prepend(PablicusMessageMenu.icon(icon));b.onclick=run;host.append(b);};
  const unread=!!(+d.unread_count||d.marked_unread);
  action(before,'read',unread?'Прочитать':'Не прочитан',unread?'check':'chats',()=>changeChatPreference(d,unread?'read':'unread'));
  action(before,'pin',d.pinned?'Открепить':'Закрепить','pin',()=>changeChatPreference(d,'pin',!d.pinned));
  action(after,'mute',d.muted?'Вкл. звук':'Без звука','notifications',()=>changeChatPreference(d,'mute',!d.muted));
  action(after,'delete','Удалить','delete',()=>confirmChatDelete(d));
  action(after,'archive',d.archived?'Из архива':'В архив','archive',()=>changeChatPreference(d,'archive',!d.archived));
  button.setAttribute('aria-keyshortcuts','ArrowLeft ArrowRight');button.setAttribute('aria-description','Смахните вправо для прочтения и закрепления, влево для звука, удаления и архива.');
  row.append(before,after,face);chatListGestures.decorate(row,face,before,after);
 }
 function renderProfileSettings(parent){
   ...
   const shareAppButton=el('button','setting profileShare','Пригласить в Pablicus');shareAppButton.id='shareApp';shareAppButton.type='button';shareAppButton.prepend(PablicusMessageMenu.icon('share'));shareAppButton.onclick=async()=>{shareAppButton.disabled=true;try{await personalInvites.open()}finally{shareAppButton.disabled=false}};p.append(shareAppButton);
   ...
   const info=el('p','muted','Pablicus '+VERSION+' · Кандидат выпуска. Черновики и исходящие сохраняются отдельно для каждого аккаунта и разговора.');p.append(installB,logout,info);parent.append(p);mountStorageUsage(p);pushNotifications.mount(p);contactDiscovery.mount(p);joinedNotices.mount(p);passkeySettings(p);
 }
 function renderHome(){
  if(!user||current)return;if(page==='chats'&&chatListGestures?.isTracking()){chatListRenderPending=true;return;}chatListRenderPending=false;const openActions=chatListGestures?.snapshot();resetTasksHome();const c=$('screenContent');if(page==='profile'&&profilePage.mounted(c))return;profilePage.destroy();if(page!=='chats')c.replaceChildren();window.PablicusShell.project(window.PablicusController.state());
  if(page!=='chats')disposeChatList();
  if(page==='chats'){
   ensureChatList();
 ...
 if(window.PablicusController){
  window.PablicusController.register('home',async({state,isCurrent})=>{if(!isCurrent())return;await leaveConversationView(isCurrent);if(!isCurrent())return;page=state.section;renderHome();return()=>{profilePage.destroy()};});
 ...
 function messageImagePaths(list){const out=[];for(const raw of list||[]){const m=PablicusChatActions.effective(raw),md=m.attachment_metadata||{};if(m.type==='image'&&m.attachment_path)out.push(m.attachment_path);if(m.type==='rich'&&Array.isArray(md.blocks))for(const b of md.blocks)if(b.type==='image'&&b.path)out.push(b.path);}return out}
 function openConversation(d,options={}){return goConversation(d,options)}
 async function mountConversation(d,{canvas=false,isCurrent=()=>true}={}){if(opening||!user||!isCurrent())return;workspaceQuick.close();resetTasksHome();resetConversationView();if(canvas){canvasVisible=true;paintConversationView();}mediaViewer.close();chatLibrary.reset();messageTools.clear();PablicusRichMessage.stopAll?.();opening=true;const uid=user.id,ep=++epoch;current=d;inboxContext();rows=[];peersRead=0;if(channel){await sb.removeChannel(channel);channel=null}window.PablicusShell.conversationReady(false);window.PablicusShell.show('conversation');window.PablicusShell.conversationTitle(d.title);connection();$('chatHint').textContent='Загружаем разговор…';$('chatHint').hidden=false;try{
   if(chatHidden(d)){const restored=await sb.rpc('pablicus_chat_preferences',{p_action:'restore',p_conversation_id:d.id,p_value:null});if(restored.error)throw restored.error;if(user?.id!==uid||ep!==epoch||!isCurrent())return;chatPrefs.set(d.id,restored.data);d=withChatPreference(d);current=d;dialogs=dialogs.map(item=>item.id===d.id?d:item);safeSet(cacheKey(),dialogs);}

   const cachedRows=safeGet(messageCacheKey(d.id))||[];let paintedCache=false;
   if(cachedRows.length){rows=cachedRows.slice(-150);await PablicusMediaCache.hydrateAuthorized(BUCKET,messageImagePaths(rows),{width:960,leaseSeconds:90}).catch(()=>{});if(!isCurrent()||ep!==epoch||user?.id!==uid)return;await PablicusChat.open(uid,d.id,rows.map(mapped));paintedCache=true;window.PablicusShell.conversationReady(true);$('chatHint').hidden=true;}
   let remote=[];const r=navigator.onLine?await sb.from('messages').select('*').eq('conversation_id',d.id).order('server_seq',{ascending:false}).limit(150):{data:[],error:null};if(r.error){if(navigator.onLine)toast('История пока недоступна. Черновик и очередь доступны локально.')}else remote=(r.data||[]).reverse();if(!isCurrent()||ep!==epoch||user?.id!==uid){if(isCurrent()&&current?.id===d.id&&user?.id===uid){window.PablicusShell.conversationReady(true);}return;}if(remote.length||!paintedCache){rows=remote;if(remote.length)safeSet(messageCacheKey(d.id),remote);inboxSeq.set(d.id,Math.max(inboxSeq.get(d.id)||0,+rows.at(-1)?.server_seq||0));await PablicusMediaCache.hydrateAuthorized(BUCKET,messageImagePaths(rows),{width:960,leaseSeconds:90}).catch(()=>{});if(!isCurrent()||ep!==epoch||user?.id!==uid)return;if(paintedCache)PablicusChat.update(rows.map(mapped));else await PablicusChat.open(uid,d.id,rows.map(mapped));}if(!isCurrent()||ep!==epoch||user?.id!==uid){if(isCurrent()&&current?.id===d.id&&user?.id===uid){window.PablicusShell.conversationReady(true);}return;}window.PablicusShell.conversationReady(true);connection();
  ...
  }catch(e){if(!isCurrent()||ep!==epoch||user?.id!==uid)return;problem(e);if(current?.id===d.id){window.PablicusShell.conversationReady(true);}current=null;window.PablicusShell.show('home');renderHome()}finally{if(ep===epoch&&user?.id===uid){if(isCurrent()&&current?.id===d.id){window.PablicusShell.conversationReady(true);}opening=false;inboxContext();workspaceQuick.refresh()}}}
 async function leaveConversationView(isCurrent=()=>true){
  ...
  if(!isCurrent())return;
  current=null;opening=false;inboxContext();epoch++;if(channel)sb.removeChannel(channel);channel=null;window.PablicusShell.show('home');
 }
 $('chatBack').onclick=async()=>{if(window.PablicusChat?.composer?.back())return;try{if(window.PablicusController){await window.PablicusController.navigate({section:page==='bots'?'chats':page,screen:'home',conversationId:null,resourceId:null,canvas:false});return}if(!canvasNavigationAllowed())return;await leaveConversationView();renderHome();loadDialogs()}catch(e){problem(e)}};
 async function syncMessages(){if(!current||!user||syncing||opening||document.hidden||!navigator.onLine)return;syncing=true;const d=current,uid=user.id,ep=epoch;try{
  ...
  if(r.data?.length){const seen=new Set(rows.map(m=>m.id));rows.push(...r.data.filter(m=>!seen.has(m.id)));safeSet(messageCacheKey(d.id),rows.slice(-150));PablicusChat.update(rows.map(mapped))}
  inboxSeq.set(d.id,Math.max(inboxSeq.get(d.id)||0,+rows.at(-1)?.server_seq||0));
  if(!canvasVisible&&PablicusChat.list?.follow&&rows.length)await sb.rpc('mark_conversation_read',{p_conversation_id:d.id,p_last_read_seq:rows.at(-1).server_seq});
 ...
 function replyDescription(target,ref){target=target?PablicusChatActions.effective(target):target;const block=ref.block_id?target?.attachment_metadata?.blocks?.find(b=>b.id===ref.block_id):null,type=block?.type||target?.type;
  ...
  const text=block?(block.type==='text'?block.text:PablicusRichMessage.attachmentLabel(block)):(target?.body||labels[type]||'Сообщение');
 ...
 async function locateMessage(message){
  const uid=user?.id,cid=current?.id;if(!uid||message.conversation_id!==cid)return;
  if(canvasVisible&&await showConversationView()===false)return;
  if(!rows.some(r=>r.id===message.id)){
   // Replace the loaded window instead of merging disjoint ranges: old+latest
   // ranges would hide the gap forever from both history and live catch-up.
   const ep=++epoch,r=await sb.from('messages').select('*').eq('conversation_id',cid).gte('server_seq',Math.max(1,Number(message.server_seq)-30)).lte('server_seq',Number(message.server_seq)+30).order('server_seq',{ascending:true}).limit(61);
   if(user?.id!==uid||epoch!==ep)return;if(r.error)throw r.error;
   if(!(r.data||[]).some(m=>m.id===message.id&&!m.deleted_at))throw Error('Сообщение удалено или больше недоступно');
   rows=(r.data||[]).filter(m=>!m.deleted_at);PablicusChat.update(rows.map(mapped));messageTools.sync();
  }
  const list=PablicusChat.list,index=list?.messages.findIndex(m=>m.id===message.id);if(index<0||index==null)throw Error('Не удалось найти сообщение');list.go(index);
  const ep=epoch;requestAnimationFrame(()=>requestAnimationFrame(()=>{if(user?.id!==uid||epoch!==ep)return;const node=[...document.querySelectorAll('#canvas .row')].find(n=>n.dataset.id===message.id);if(node){node.classList.add('locatedMessage');setTimeout(()=>node.classList.remove('locatedMessage'),1900);}}));
 }
 ...
 async function composerSources(){
  ...
  const result=await PablicusAI.sources({conversationId:cid||null});
  if(user?.id!==uid||current?.id!==cid)return[];return result;
 }
 async function composerAction(kind){
  await PablicusChat.persistDraft();
  if(kind==='bots'||kind==='factory')return PablicusController.navigate({section:'bots',screen:kind});
  if(!current)throw Error('Сначала откройте разговор.');
  if(kind==='task')return showCanvasView({title:'',content:{v:1,blocks:[]}});
  if(kind==='project'){await showCanvasView();return chatCanvas.editProject();}
  if(kind==='canvas')return showCanvasView();
  throw Error('Действие пока не подключено.');
 }
 window.PablicusHost={composerTarget:()=>current?{id:current.id,label:current.title||'Текущий чат'}:null,composerSources,composerAction,renderMessage,renderPendingMessage,messageMeta,paintReplyDraft,notify:toast,acceptFile,unavailable,showOutbox,historyTop:loadOlder,canSend:()=>{if(!user||!profile?.is_approved||!current||opening||PablicusChat.scope.user!==user.id||PablicusChat.scope.chat!==current.id)return false;if((PablicusChat.rich?.capture().text||$('input').value||'').length>5000){toast('Одно сообщение — до 5000 символов. Черновик сохранён; сократите его или разделите.');return false}return true}};
 window.addEventListener('pablicus:queued',()=>pump());window.addEventListener('online',()=>{connection();dialogsRequest=null;loadDialogs();syncMessages();pump()});window.addEventListener('offline',connection);document.addEventListener('visibilitychange',()=>{inboxContext();if(!document.hidden)messageTools.sync();if(!document.hidden){connection();dialogsRequest=null;loadDialogs();syncMessages();pump()}});
 setInterval(()=>{if(!document.hidden){pollInbox();messageTools.sync()}},5000);
 setInterval(()=>{if(!document.hidden){if(current)syncMessages();else if(Date.now()-dialogsLast>=15000)loadDialogs();pump()}},2500);connection();
