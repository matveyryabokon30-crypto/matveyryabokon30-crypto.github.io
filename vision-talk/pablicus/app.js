...
(() => {'use strict';
 ...
 callPanel.addEventListener('cancel',e=>{e.preventDefault();void calls.end();callPanel.close();});
 function startContactCall(id,kind){if(!profile?.is_approved||!user)return false;const s=calls.snapshot();if(!s.available){toast('Звонки пока не подключены.');return false;}if(s.call||s.phase==='checking'){toast('Сначала завершите текущий звонок.');return false;}void calls.start(id,kind);return true;}
 ...
 window.PablicusController.setServices({...window.PablicusController.getServices(),beforeNavigate:appNavigationAllowed,openConversation,startContactCall,getDialogs:()=>dialogs,getUser:()=>user,getProfile:()=>profile,notify:toast});
}
