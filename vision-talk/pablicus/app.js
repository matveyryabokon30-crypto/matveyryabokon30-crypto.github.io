...
(() => {'use strict';
 ...
 function renderHome(){
  if(!user||current)return;if(page==='chats'&&chatListGestures?.isTracking()){chatListRenderPending=true;return;}chatListRenderPending=false;const openActions=chatListGestures?.snapshot();resetTasksHome();const c=$('screenContent');c.replaceChildren();window.PablicusShell.project(window.PablicusController.state());
