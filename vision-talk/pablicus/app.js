...
(() => {'use strict';
 const URL='https://ctcoqgsztdtsazdiwcmd.supabase.co',KEY='sb_publishable_kMGqZAM2vadfXbBr8r5uzw_l9EiBtIw',BUCKET='message-media',VERSION='P05';
 ...
 async function mountConversation(d,{canvas=false,isCurrent=()=>true}={}){if(opening||!user||!isCurrent())return;workspaceQuick.close();resetTasksHome();resetConversationView();if(canvas){canvasVisible=true;paintConversationView();}mediaViewer.close();chatLibrary.reset();messageTools.clear();PablicusRichMessage.stopAll?.();opening=true;const uid=user.id,ep=++epoch;current=d;inboxContext();rows=[];peersRead=0;if(channel){await sb.removeChannel(channel);channel=null}window.PablicusShell.conversationReady(false);window.PablicusShell.show('conversation');window.PablicusShell.conversationTitle(d.title);connection();$('chatHint').textContent='Загружаем разговор…';$('chatHint').hidden=false;try{
   ...
   let remote=[];const r=navigator.onLine?await (async()=>{try{const boot=await PablicusScreenData.history(d.id);return{data:boot.messages,error:null};}catch{if(!isCurrent()||user?.id!==uid)return{data:[],error:null};return await sb.from('messages').select('*').eq('conversation_id',d.id).order('server_seq',{ascending:false}).limit(150);}})():{data:[],error:null};if(r.error){if(navigator.onLine)toast('История пока недоступна. Черновик и очередь доступны локально.')}else remote=(r.data||[]).reverse();if(!isCurrent()||ep!==epoch||user?.id!==uid){if(isCurrent()&&current?.id===d.id&&user?.id===uid){window.PablicusShell.conversationReady(true);}return;}rows=remote;inboxSeq.set(d.id,Math.max(inboxSeq.get(d.id)||0,+rows.at(-1)?.server_seq||0));await PablicusChat.open(uid,d.id,rows.map(mapped));await window.PablicusScreenData.prepareConversation(()=>isCurrent()&&ep===epoch&&user?.id===uid);if(!isCurrent()||ep!==epoch||user?.id!==uid){if(isCurrent()&&current?.id===d.id&&user?.id===uid){window.PablicusShell.conversationReady(true);}return;}window.PablicusShell.conversationReady(true);window.PablicusScreenData.revealConversation();connection();
 ...
 function renderMessage(m){const r=PablicusChatActions.effective(m.remote),row=el('article','row'+(m.mine?' mine':'')),b=el('div','bubble'),t=el('div','text');row.dataset.id=m.id;row.dataset.rev=m.revision;row.dataset.serverSeq=r.server_seq;row.dataset.created=r.created_at;row.dataset.edited=r.edited_at||'';
  ...
  if(r.type==='rich')b.append(renderRichContent(r.attachment_metadata,false,r));
   ...
   if(r.type==='image'){const im=el('img','messageImage');im.alt='Фото';im.loading='eager';im.decoding='sync';if(/^data:image\/(jpeg|png|webp);base64,/.test(md.thumb_data_url||''))im.src=md.thumb_data_url;button.append(im);if(!im.src){const fallback=el('span','mediaLabel','Фото');button.append(fallback);requestAnimationFrame(()=>{if(!im.isConnected||!im.closest('#canvas'))return;im._pablicusReady=signedUrl(r.attachment_path,{type:'image',width:960}).then(async u=>{if(im.isConnected){im.src=u;await im.decode();if(im.isConnected)fallback.remove();}}).catch(()=>{fallback.textContent='Фото · нажмите, чтобы повторить'})})}}
 ...
 setInterval(()=>{if(!document.hidden){if(current)syncMessages();else if(Date.now()-dialogsLast>=10000)loadDialogs();pump()}},1300);connection();
