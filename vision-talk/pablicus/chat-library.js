...
(function (scope) {
  ...
  function material(value) {
    return {
      ...
      name: scope.PablicusRichMessage.attachmentLabel({type:value.type,name:value.name || value.attachment_metadata?.name}),
  ...
  function create(options) {
    ...
    function renderMessage(value) {
      ...
      highlighted(snippet, value.snippet || value.body || value.text || value.attachment_metadata?.blocks?.map(block => block.type === 'text' ? block.text : scope.PablicusRichMessage.attachmentLabel(block)).join('\n') || 'Сообщение');
