...
const text=document.createElement('div');text.className='text';text.textContent=PablicusRichMessage.attachmentLabel(a||{});
const meta=document.createElement('div');meta.className='meta';meta.textContent='Только на этом устройстве'+(a?.kind==='video'?' · без воспроизведения':'');bubble.append(slot,text,meta);row.append(bubble);return decorateQueueRow(row,m);
}
...
let fullEntry=null,viewWasFull=false,r2Composer=null;
let mirror=null,mirrorText=null,mirrorWidth=0,mirrorHeight=40;
function naturalInputHeight(){if(richComposer)return Math.max(r2Composer?24:40,richComposer.height);
...
if(!r2Composer)$('expand').textContent=draft.expanded?'↙':'⛶';$('expand').setAttribute('aria-expanded',String(draft.expanded));$('expand').setAttribute('aria-label',draft.expanded?'Вернуться в чат':'Редактор на весь экран');
...
if(!r2Composer)$('send').textContent=draft.mode==='message'?'↑':'▷';$('send').setAttribute('aria-label',draft.mode==='message'?'Отправить сообщение':'Запустить демонстрацию, без ИИ');
$('send').disabled=!list||draft.composing||richComposer?.composing||!!richComposer?.recording||!!richComposer?.pending||!hasComposerContent();
if(draft.expanded){
...
const shellInsets=(parseFloat(shellStyle.paddingTop)||0)+(parseFloat(shellStyle.paddingBottom)||0);
// Grow with the actual lines until the visible screen is full, leaving a slice
// of history. Use the same limit for the textarea and its host so native caret
// scrolling stays inside the editor. Recompute before measuring on resize.
const cap=Math.max(r2Composer?(parseFloat(getComputedStyle(input).lineHeight)||24):44,Math.min(r2Composer?(richComposer?.capture().files.length?240:6*(parseFloat(getComputedStyle(input).lineHeight)||24)):Infinity,app.clientHeight-shellInsets-chromeHeight-overhead-64));
$('editor').style.setProperty('--composer-input-cap',cap+'px');
...
if(r2Composer&&document.activeElement===input){const editor=$('editor'),bottom=input.offsetTop+input.offsetHeight-editor.offsetTop;if(bottom>editor.scrollTop+editor.clientHeight)editor.scrollTop=bottom-editor.clientHeight;}

}
r2Composer?.sync({expanded:draft.expanded,busy:submitBusy});
cm.layout_updates++;
...
const active=document.activeElement,panel=active?.closest('.r2Panel:not([hidden])');
// Portalled composer fields belong to the same keyboard layout as the draft.
const owned=active&&(app.contains(active)||(panel&&app.querySelector('[aria-controls="'+panel.id+'"]')));
...
function addAssetNode(a){const n=document.createElement('div');n.className='attachment';n.dataset.asset=a.id;const glyph=document.createElement('span');glyph.className='fileGlyph';glyph.textContent=a.kind==='image'?'▧':a.kind==='video'?'▷':'▤';const label=document.createElement('span');label.className='label';label.textContent=a.kind==='document'?PablicusRichMessage.attachmentLabel(a):'Готовлю превью…';const b=document.createElement('button');b.type='button';b.className='remove';b.textContent='×';b.setAttribute('aria-label','Удалить вложение '+PablicusRichMessage.attachmentLabel(a));b.addEventListener('pointerdown',e=>{if(document.activeElement===input)e.preventDefault()});b.onclick=()=>removeAsset(a.id);n.append(glyph,label,b);$('tray').append(n)}
function assetLabel(a){return a.kind==='video'?(a.duration?Math.floor(a.duration/60)+':'+String(Math.round(a.duration%60)).padStart(2,'0')+' · видео':'Видео'):(a.kind==='image'?'Фото':PablicusRichMessage.attachmentLabel(a))}
...
n.querySelector('.label').textContent=assetLabel(a)+(a.state==='error'?' · без превью':'');n.title=PablicusRichMessage.attachmentLabel(a)+(a.state==='error'?' — превью недоступно, файл остаётся в черновике':'');
}
...
document.addEventListener('focusin',queueLayout);document.addEventListener('focusout',queueLayout);
...
function ensureRichComposer(){if(richComposer)return;richComposer=PablicusRichComposer.create({
 ...
 r2Composer=PablicusComposerR2.create({root:$('composeBox'),body:$('editor'),input,attach:$('attach'),voice:richComposer.voiceButton,send:$('send'),expand:$('expand'),
  collapseFull:()=>{if(draft.expanded)toggleExpand();},capture:()=>richComposer.capture(),recording:()=>richComposer.recording,getScope:()=>({user:scopeUser,chat:scopeChat}),onGeometry:queueComposer,onChange:draftChanged,onContext:draftChanged,
  pick:accept=>{const picker=$('documentInput');picker.accept=accept;picker.value='';picker.click();},
  sources:()=>window.PablicusHost.composerSources(),action:kind=>window.PablicusHost.composerAction(kind)});
 richComposer.resize();
}
window.PablicusChat={
 ...
 async leave(){r2Composer?.close(false);await this.flush();input.blur();closeMenu(false);list?.destroy();list=null;},
 appendContent,setReply,get reply(){return replyTarget},get composer(){return r2Composer},get rich(){return richComposer},
