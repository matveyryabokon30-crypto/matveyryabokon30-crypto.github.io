...
(function (scope) {
  ...
  function create(options = {}) {
    ...
    let rich, chrome, destroyed = false, disabled = false, expanded = false, restoring = false, base = '', remote = new Map();
    ...
    function report(message) {
      ...
      chrome?.sync({expanded,busy:disabled});
    ...
    function decorateRemote() {
      ...
      for (const card of body.querySelectorAll('.richMedia')) {
        ...
        if (name) name.textContent = scope.PablicusRichMessage.attachmentLabel(block);
        ...
        const open = button(`Повторить загрузку ${scope.PablicusRichMessage.attachmentLabel(block)}`, 'Повторить', async () => {
        ...
        if (player) {
          player.hidden = true;
          if (block.type === 'image') { player.alt = 'Фото'; player.loading = 'lazy'; }
    ...
    chrome=scope.PablicusComposerR2.create({root,body,input,attach,voice:rich.voiceButton,expand,collapseFull:()=>setExpanded(false),capture:()=>rich.capture(),recording:()=>rich.recording,getScope:()=>({editor:root.id,epoch:contentEpoch}),onGeometry:options.onGeometry,
      pick:accept=>{picker.accept=accept;picker.value='';picker.click();},sources:()=>scope.PablicusHost?.composerSources?.()||[],action:kind=>{if(rich.recording||pendingAdds||fingerprint(rawSnapshot())!==base)throw Error('Сохраните текущие материалы перед переходом. Текст и файлы остаются в редакторе.');return scope.PablicusHost?.composerAction?.(kind);},onContext:changed});
    rich.resize();
    ...
    function setContent(content, settings = {}) {
      ...
      try {
        ...
        rich.restore({blocks, files, selection: settings.selection});chrome?.restore(settings.composer);
    ...
    const api = {
      ...
      destroy() {
        if (destroyed) return;
        destroyed = true; contentEpoch++; chrome?.destroy();rich.destroy(); root.remove();
