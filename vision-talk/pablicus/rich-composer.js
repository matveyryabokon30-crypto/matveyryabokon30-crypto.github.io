...
(function (global) {
  ...
  const mimeFor = file => {
    ...
    if (supplied === 'video/x-m4v') return 'video/mp4';
  ...
  function sizeText(element) {
    if (!element) return;
    const minimum=element.closest('.r2Composer')?24:40;
    // Measure outside the scrolled editor: WebKit can retain the old scroll
    // extent while the keyboard is open, even after deleting all its text.
    const style = getComputedStyle(element), width = element.getBoundingClientRect().width;
    if (!width) return;
    const measure = document.createElement('textarea');
    measure.tabIndex = -1;
    measure.setAttribute('aria-hidden', 'true');
    for (const key of ['font','letter-spacing','word-spacing','text-indent','white-space','word-break','overflow-wrap','padding','border','box-sizing'])
      measure.style.setProperty(key, style.getPropertyValue(key), 'important');
    for (const [key,value] of Object.entries({position:'fixed',left:'0',top:'0',width:width+'px',height:'0',minHeight:'0',maxHeight:'none',visibility:'hidden',overflow:'hidden',pointerEvents:'none'}))
      measure.style.setProperty(key.replace(/[A-Z]/g,c=>'-'+c.toLowerCase()),value,'important');
    measure.value = element.value;
    document.body.append(measure);
    const height = Math.max(minimum, measure.scrollHeight);
    measure.remove();
    element.style.height = height+'px';
    if (!element.value) element.scrollTop = 0;
  }
  ...
  function create({container, input, onChange = () => {}, onError = () => {}, onGeometry = () => {}, acceptFile} = {}) {
    ...
    let geometryFrame = 0, resizeFrame = 0, resizeObserver = null, mediaViewer = null;
    ...
    function mediaNode(block) {
      ...
      label.textContent = block.pending ? 'Идёт запись голоса' : global.PablicusRichMessage.attachmentLabel({...file, type:block.type});
      head.append(label);
      if (block.type === 'document' && global.PablicusIcons) {
        const glyph = global.PablicusIcons.icon('file');
        glyph.classList.add('richFileIcon');
        head.prepend(glyph);
      }
      ...
      if(global.PablicusIcons)global.PablicusIcons.decorate(remove,'close',remove.getAttribute('aria-label'));
      head.append(remove);
      card.append(head);
      if (block.pending) {
        ...
        if (block.type === 'image') {
          ...
          img.addEventListener('error', () => {
            img.hidden = true;
            label.textContent = 'Фото · предпросмотр недоступен';
            geometry();
          }, {once: true});
          card.append(img);
        } else if (block.type === 'audio') {
          const audio = document.createElement('audio');
          audio.controls = !global.PablicusIcons;
          audio.preload = 'none';
          audio.src = objectURL(file);
          audio.setAttribute('aria-label', 'Прослушать голосовое сообщение');
          card.append(audio);
          if(global.PablicusIcons){const preview=document.createElement('div');preview.className='r2AudioPreview';const time=document.createElement('span');time.className='r2AudioTime';time.textContent='00:00';const play=global.PablicusIcons.button('play','Прослушать запись',()=>{if(audio.paused)audio.play().catch(()=>error('Не удалось воспроизвести запись. Файл сохранён.'));else{audio.pause();audio.currentTime=0;}});const paint=()=>{global.PablicusIcons.decorate(play,audio.paused?'play':'stop',audio.paused?'Прослушать запись':'Остановить прослушивание');const sec=Math.floor(audio.currentTime||0);time.textContent=String(Math.floor(sec/60)).padStart(2,'0')+':'+String(sec%60).padStart(2,'0');};for(const event of ['play','pause','ended','timeupdate'])audio.addEventListener(event,paint);preview.append(play,time);card.append(preview);card.disposeMedia=()=>{for(const event of ['play','pause','ended','timeupdate'])audio.removeEventListener(event,paint);audio.pause();audio.removeAttribute('src');audio.load();};}
    ...
    function preparePreviews() {
      for (const media of container.querySelectorAll('.richMedia img,.richMedia video')) {
        media.tabIndex = 0;
        media.setAttribute('role', 'button');
        media.setAttribute('aria-label', media.tagName === 'IMG' ? 'Посмотреть фото' : 'Посмотреть видео');
        if (media.tagName === 'VIDEO') media.controls = false;
      }
    }
    function showPreview(event) {
      if (event.type === 'keydown' && !['Enter',' '].includes(event.key)) return;
      const media = event.target.closest('.richMedia img,.richMedia video');
      if (!media || !container.contains(media) || !media.src || !global.PablicusMediaViewer) return;
      event.preventDefault();
      mediaViewer ||= global.PablicusMediaViewer.create({resolveUrl: item => item.url, onError: () => error('Не удалось открыть вложение.')});
      mediaViewer.open([{type:media.tagName === 'IMG' ? 'image' : 'video',url:media.src}]);
    }
    listen(container, 'click', showPreview);
    listen(container, 'keydown', showPreview);
    function render(restoreFocus = true) {
      if (destroyed) return;
      const hadFocus = container.contains(document.activeElement);
      const liveIds = new Set(blocks.map(block => block.id));
      for (const [id, node] of nodes) if (!liveIds.has(id)) { composing.delete(node); textListeners.get(node)?.();node.disposeMedia?.(); node.remove(); nodes.delete(id); }
      ...
      blocks.forEach((block, index) => {
        ...
        if (!node) {
          node = block.type === 'text' ? bindText(blocks.find(b=>b.type==='text')===block ? input : document.createElement('textarea'), block) : mediaNode(block);
    ...
    function insert(media, insertion = selected) {
      readText();
      if(container.closest('.r2Composer')){
        // R2 keeps attachments above a single text field. Adding a file never
        // splits the sentence at the caret or creates empty continuation fields.
        const text=blocks.find(b=>b.type==='text');
        blocks=[...blocks.filter(b=>b.type!=='text'),...media,text];
        selected={...insertion,blockId:text.id};render();focus();changed();return;
      }
    ...
    async function addFiles(incoming) {pendingFiles++;changed();try{return await addFilesNow(incoming);}finally{pendingFiles--;changed();}}
    async function addFilesNow(incoming) {
      ...
      for (const file of Array.from(incoming || [])) {
        ...
        const id = uid(), kind = kindFor(file), type = mimeFor(file);
        // Multipart upload uses the Blob MIME, not just the separate metadata.
        // iOS Files can return MOV/MP4 with an empty or generic MIME.
        const normalized = file.type === type ? file : new File([file], file.name || 'Файл', {type, lastModified:file.lastModified || Date.now()});
        accepted.push({id, name: normalized.name, type, kind, size: file.size, lastModified: normalized.lastModified, file:normalized});
    ...
    function reset(snapshot = {}) {
      ...
      urls.clear();
      for (const node of nodes.values()) if (node !== input) { textListeners.get(node)?.();node.disposeMedia?.(); node.remove(); }
      nodes.clear();
      ...
      if (!container.closest('.r2Composer') && blocks[0]?.type !== 'text') blocks.unshift(textBlock());
      if (blocks.at(-1)?.type !== 'text') blocks.push(textBlock());
      // A restored draft containing only blank lines is an empty message, not
      // a request to reopen a tall editor. Normalize only on restore: live
      // Enter/Backspace must still grow and shrink the field line by line.
      // Any real text or media keeps the full ordered document unchanged.
      if (blocks.every(block => block.type === 'text' && !block.text.trim())) {
        blocks = [{...blocks[0], text: ''}];
      }
      if(container.closest('.r2Composer')){
        const texts=blocks.filter(b=>b.type==='text'),first=texts[0]||textBlock();
        const prior=blocks.find(b=>b.id===snapshot.selection?.blockId&&b.type==='text');
        const offset=prior?texts.slice(0,texts.indexOf(prior)).reduce((n,b)=>n+b.text.length+1,0):0;
        first.text=texts.map(b=>b.text).join('\n');
        snapshot={...snapshot,selection:{...snapshot.selection,blockId:first.id,start:offset+(snapshot.selection?.start||0),end:offset+(snapshot.selection?.end||0)}};
        blocks=[...blocks.filter(b=>b.type!=='text'),first];
      }
      ...
      selected = {...(snapshot.selection || {}), blockId: blocks.some(block => block.type === 'text' && block.id === snapshot.selection?.blockId) ? snapshot.selection.blockId : blocks.find(b=>b.type==='text').id};
      ...
      input.value = blocks.find(b=>b.type==='text').text;
      render(false);
    ...
    function paintVoice() {
      ...
      if(global.PablicusIcons)global.PablicusIcons.decorate(voiceButton,active?'stop':'microphone',voiceButton.getAttribute('aria-label'));else{microphone.hidden=active;voiceStop.hidden=!active;}
    }
    ...
    paintVoice();
    return {
      resize(){for(const node of nodes.values())if(node.tagName==='TEXTAREA')sizeText(node);geometry();},
      capture, restore: reset, clear() { reset(); changed(); }, addFiles, focus, blur, voiceButton, preparePreviews,
      startRecording, stopRecording, restoreSelection,
      get selection() { return {...selected}; },
      get pending(){return pendingFiles>0;},
      get composing() { return composing.size > 0; },
      get recording() { return Boolean(recording || requesting); },
      get height() {
        const boxes = [...nodes.values()].map(node => node.getBoundingClientRect());
        if (!boxes.length) return 0;
        // Adjacent attachment cards share a row. Measure the occupied extent,
        // not the sum of individual heights, including after wrapping/removal.
      ...
      destroy() {
        ...
        mediaViewer?.destroy();
        revision++;
        for(const node of nodes.values())node.disposeMedia?.();
  ...
  global.PablicusRichComposer = {create, sizeText, kindFor, mimeFor, MAX_FILE_BYTES};
