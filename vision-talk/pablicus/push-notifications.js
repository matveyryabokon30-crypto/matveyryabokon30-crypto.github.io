...
(function(root){
 ...
 function limited(promise,ms=8000){let timer;return Promise.race([promise,new Promise((_,reject)=>{timer=setTimeout(()=>reject(Object.assign(Error('Нет ответа. Проверьте интернет и повторите.'),{code:'timeout'})),ms);})]).finally(()=>clearTimeout(timer));}
 function create(options={}){
  ...
  let installationId=null,trackingState="unknown",healthFlight=null,healthNext=0,lastError='none';
  ...
  function view(message=''){
   ...
   status.textContent=message||(enabled?'Подписка подключена. Доставка на экран ещё не подтверждена.':'Сообщения и напоминания о делах смогут появляться на экране, даже когда приложение закрыто.');
  }
  ...
  function installationMetadata(uid){
   ...
   try{const response=await limited(fetch(endpoint,{method,headers:{Authorization:'Bearer '+session.access_token,apikey:options.apiKey,'Content-Type':'application/json'},...(body?{body:JSON.stringify(body)}:{}),signal:controller.signal,cache:'no-store'}),11000);
    ...
    if(!response.ok)throw Object.assign(Error(response.status===401?'Войдите снова, чтобы включить уведомления.':'Не удалось подключить уведомления. Повторите попытку.'),{code:response.status===401?'auth':'service'});
  ...
  async function reconcile(uid,token,seq){
   ...
   // An existing consented subscription must regain its local recipient even if config is offline.
   if(sub&&readOwner()===uid){await bind(uid,reg);guard(uid,token);}
   ...
   saveConsent(uid,'enabled');writeOwner(uid);enabled=true;phase='ready';lastError='none';failures=0;nextAt=Date.now()+60000;view();return result;
  }
  function refresh(){
   const uid=options.getUserId();
   if(uid!==account){invalidate();account=uid||null;healthNext=0;enabled=false;installationId=null;}
   if(refreshing)return refreshing;
   if(!uid||destroyed||suspended||paused(uid)||busy||!supported()||(needsInstall()&&!standalone())){phase=!uid?'no_account':suspended||paused(uid)?'paused':!supported()?'unsupported':needsInstall()&&!standalone()?'not_standalone':phase;return Promise.resolve();}
   ...
   refreshing=limited(work,35000).catch(e=>{
    if(still(uid,token)){lastError=['auth','service','timeout','storage','worker'].includes(e.code)?e.code:e.name==='TypeError'?'network':'unknown';for(const c of controllers)c.abort();++generation;enabled=false;phase=e.message==='PUSH_OPERATION_CANCELLED'?'cancelled':'retry';failures++;nextAt=Date.now()+Math.min(300000,2000*2**(failures-1))*(0.8+Math.random()*0.4);view(e.message==='PUSH_OPERATION_CANCELLED'?'':e.message);schedule();}
  ...
  async function localHealth(){
   const uid=options.getUserId(),token=generation;
   let saved=null;try{saved=root.localStorage.getItem(consentKey(uid));}catch{}
   const health={permission:root.Notification?.permission||'unsupported',consent:saved==='disabled'?'disabled':consent(uid)?'enabled':'unknown',owner:readOwner()===uid?'current':readOwner()?'other':'missing',standalone:!!standalone(),web_locks:!!navigator.locks?.request,subscription:'unknown',registration:'unknown',worker_build:null,phase,error:lastError};
   try{const reg=await limited(navigator.serviceWorker?.getRegistration(scope),2500);
    health.registration=!reg?.active?'missing':reg.scope===scope?'current':'mismatch';
    if(reg?.active&&reg.scope===scope){
     health.subscription=await limited(reg.pushManager.getSubscription(),2500)?'present':'missing';
     health.worker_build=await new Promise(resolve=>{const ch=new MessageChannel();let done=false;const finish=v=>{if(done)return;done=true;clearTimeout(timer);ch.port1.close();ch.port2.close();resolve(/^git:[a-f0-9]{40}$/.test(v||'')?v:null);};const timer=setTimeout(()=>finish(null),2000);ch.port1.onmessage=e=>finish(e.data?.buildId);try{reg.active.postMessage({type:'PABLICUS_RELEASE'},[ch.port2]);}catch{finish(null);}});
    }
   }catch{}
   return still(uid,token)?health:null;
  }
  async function reportHealth(){
   if(!options.enableHealthTracking||destroyed||!options.getUserId()||navigator.onLine===false)return null;
   if(healthFlight)return healthFlight;
   if(Date.now()<healthNext)return null;
   const uid=options.getUserId(),token=generation;healthNext=Date.now()+60000;
   healthFlight=(async()=>{const health=await localHealth();if(!health||!still(uid,token))return null;const installation=installationMetadata(uid);if(!installation)return health;
    try{await request('POST',{action:'client_health',installation,health},uid,token);}catch{}return health;
   })().finally(()=>{healthFlight=null;});return healthFlight;
  }
  async function checkConnection(){
   await refresh();const health=await localHealth();if(!health)return;
   const description=health.permission==='denied'?'Уведомления запрещены в настройках iPhone.':health.consent==='disabled'?'Уведомления отключены внутри Pablicus.':health.consent==='unknown'?'На этой установке нет сохранённого согласия. Нажмите «Включить уведомления».':!health.standalone&&needsInstall()?'Откройте Pablicus с главного экрана iPhone.':health.registration!=='current'?'Обработчик этой установки ещё не готов.':health.subscription==='missing'?'Подписка отсутствует. Восстановление пока не завершено.':enabled?'Подписка подключена. Получение на экран проверяется отдельно.':'Подключение не подтверждено. Проверка не отправляет тестовых уведомлений.';
   view(description);healthNext=0;void reportHealth();return health;
  }
  function mount(container){
   ...
   button.onclick=()=>enabled?disable():enable();const check=element('button','setting','Проверить подключение');check.type='button';check.onclick=async()=>{check.disabled=true;try{await checkConnection();}finally{check.disabled=false;}};section.append(title,button,check,status);container.append(section);view();refresh();return section;
  }
  ...
  function sessionRestored(session){const uid=options.getUserId();if(session?.user?.id===uid)lastSession=session;const sid=sessionIdentity(session);if(uid)try{const prior=root.localStorage.getItem(pauseKey(uid));if(prior&&sid&&prior!=='unknown'&&sid!==prior)root.localStorage.removeItem(pauseKey(uid));}catch{}suspended=false;const pending=refresh();void pending.finally(()=>reportHealth());return pending;}
  function lifecycle(){if(!document.hidden){const pending=refresh();void pending.finally(()=>reportHealth());}}
  ...
  return{mount,refresh,sessionRestored,enable,disable,signOut,clear,destroy,inspect,localHealth,reportHealth,checkConnection,get enabled(){return enabled;}};
 }
