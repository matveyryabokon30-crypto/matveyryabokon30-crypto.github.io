  ...
  function create(options) {
    ...
    function renderTasks() {
      ...
      for (const task of tasks) {
        ...
        const toggle = button('pthIcon pthToggle', (task.completed ? 'Вернуть в работу: ' : 'Завершить: ') + scope.PablicusRichMessage.readableName(task.title, 'Дело'));
        ...
        const open = button('pthOpen', 'Открыть дело «' + scope.PablicusRichMessage.readableName(task.title, 'Дело') + '» в чате');
        const title = el('span', 'pthTaskTitle', scope.PablicusRichMessage.readableName(task.title, 'Дело'));
