...
function create(o){
 ...
 on(actions,'pointermove',e=>{const d=actionDrag;if(!d||d.id!==e.pointerId)return;if(!d.moved&&Math.hypot(e.clientX-d.x,e.clientY-d.y)<6)return;if(!d.moved){d.moved=true;actions.setPointerCapture(e.pointerId);actions.dataset.dragging='true';}e.preventDefault();const r=actions.getBoundingClientRect(),x=e.clientX-r.left;let nearest=0,distance=Infinity;for(const [i,b]of actionButtons.entries()){const delta=Math.abs(x-b.offsetLeft-b.offsetWidth/2);if(delta<distance){distance=delta;nearest=i;}}const offset=Math.max(actionOffset(0),Math.min(actionOffset(actionButtons.length-1),x-accent.offsetWidth/2));selectAction(nearest);accent.style.transform='translateX('+offset+'px)';});
 ...
 const actionResize=typeof ResizeObserver==='function'?new ResizeObserver(()=>{if(!destroyed&&!actionDrag)selectAction(selectedAction);}):null;actionResize?.observe(actions);selectAction(selectedAction);
 ...
 mark();sync();selectAction(selectedAction);return {sync,open,close,back:handleBack,get opened(){return opened;},capture:()=>({draftId,context:structuredClone(context)}),restore(value){context=structuredClone(value?.context||[]);draftId=value?.draftId||crypto.randomUUID();close(false);mark();},destroy(){full=false;close(false);syncHistory();destroyed=true;actionResize?.disconnect();life.abort();panel.remove();}};
}
