  ...
  function create(options) {
    ...
    function renderTasks() {
      ...
      for (const task of [...tasks].sort((a, b) => Number(!!a.completed) - Number(!!b.completed))) {
        ...
        const edit = button('pccTaskEdit', 'Изменить дело: ' + scope.PablicusRichMessage.readableName(task.title, 'Дело'));
        edit.textContent = '';
        edit.append(el('span', 'pccTaskText', scope.PablicusRichMessage.readableName(task.title, 'Дело')), icon('edit'));
    ...
    const api = { mount, open, close, reset, newTask, editProject, hasUnsavedChanges, destroy, element: pane };
