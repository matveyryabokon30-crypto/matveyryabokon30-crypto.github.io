...
(() => {
 ...
 class RichStore extends PablicusStore {
  async write(snapshot,expected,byteFallback=this.byteFallback===true){
   ...
   if(byteFallback){this.byteCache||=new WeakMap();for(const f of snapshot.files){let bytes=this.byteCache.get(f.file);if(!bytes){bytes=await f.file.arrayBuffer();this.byteCache.set(f.file,bytes);}encoded.set(f.id,bytes);}}
   let result;try{result=await this.transaction(['drafts','assets','outbox'],'readwrite',(tx,done,fail)=>{
    ...
    r.onsuccess=()=>{try{
     ...
     const q=tx.objectStore('outbox').getAll();q.onsuccess=()=>{try{
      ...
      for(const f of snapshot.files)if(!prior.has(f.id)&&!held.has(f.id)){as.put(encoded.has(f.id)?{id:f.id,bytes:encoded.get(f.id)}:{id:f.id,blob:f.file});puts++}
      ...
      const record={id:'draft',schema:1,revision:rev+1,savedAt:new Date().toISOString(),...content,
       composer:structuredClone(snapshot.composer||{context:[]}),selection:snapshot.selection||empty().selection,expanded:!!snapshot.expanded,intent,
    ...
    // Retry only the browser's Blob serialization failure, after the atomic
    // transaction has aborted. Quota/access/conflict failures still surface.
    if(!byteFallback&&snapshot.files.length&&e.name==='UnknownError'&&/Blob|File/.test(e.message)){this.byteFallback=true;return this.write(snapshot,expected,true);}
    throw e;
   }
    ...
    const existing=os.get(intent);existing.onsuccess=()=>{try{
     ...
     const q=ds.get('draft');q.onsuccess=()=>{try{
      ...
      for(const f of d.files){const a=as.get(f.id);a.onsuccess=()=>{try{
       const blob=DraftVault.assetBlob(a.result);if(!blob||blob.size!==f.size){fail(error('DataError','Нет полных байтов вложения; перенос остановлен'));return}
       if(--left===0)create();
      }catch(e){fail(e)}}}
     }catch(e){fail(e)}};
    }catch(e){fail(e)}};
