  ...
  const result=await this.transaction(['drafts','assets','outbox','meta'],'readwrite',(tx,done,fail)=>{
   ...
   const existing=os.get(intent);existing.onsuccess=()=>{
    ...
    const q=ds.get('draft');q.onsuccess=()=>{
     ...
     if(!left)create();
     for(const f of d.files){const a=as.get(f.id);a.onsuccess=()=>{const blob=DraftVault.assetBlob(a.result);if(!blob||blob.size!==f.size){fail(failError('DataError','Нет полных байтов вложения; перенос остановлен'));return}if(--left===0)create()};}
    };
   };
 ...
 async readQueue(withFiles=true){
  return this.transaction(['outbox','assets'],'readonly',(tx,done,fail)=>{
   const q=tx.objectStore('outbox').getAll();q.onsuccess=()=>{
    ...
    if(!withFiles)return;
    for(const r of rows.filter(active))for(const f of r.files){const a=tx.objectStore('assets').get(f.id);a.onsuccess=()=>{const blob=DraftVault.assetBlob(a.result);if(!blob||blob.size!==f.size){fail(failError('DataError','Исходящее содержит отсутствующее вложение'));return}f.file=new File([blob],f.name,{type:f.type,lastModified:f.lastModified})};}
   };
  });
 }
 async retry(id){
...
async function queueFingerprint(rows){
 const result=[];for(const r of rows.filter(active)){
  ...
  for(const m of r.messages)entry.messages.push({id:m.id,sequence:m.sequence,kind:m.kind,blocks:m.blocks,reply_to:m.reply_to||null,assetId:m.assetId||null,text:m.kind==='text'?await digest(new Blob([m.text])):null});
