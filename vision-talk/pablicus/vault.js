/* Gate 01.5 — transactional local drafts. No network, accounts or delivery. */
(() => {
'use strict';
...
const key='draft';
// Read both historical Blob records and the byte fallback for WebKit IndexedDB.
const assetBlob=row=>row?.blob instanceof Blob?row.blob:row?.bytes instanceof ArrayBuffer?new Blob([row.bytes]):null;
class DraftStore {
 constructor(name='vision-talk-gate-015-v1'){this.name=name;this.db=null;this.pendingOpen=null;this.fault=null;this.stats={commits:0,blob_puts:0,blob_deletes:0,aborts:0,conflicts:0,durability:null};}
 async open(){
  ...
  this.pendingOpen=new Promise((resolve,reject)=>{
   ...
   try{request=indexedDB.open(this.name,3)}catch(e){finish(e);return}
  ...
  return this.transaction(['drafts','assets'],'readonly',(tx,done,fail)=>{
   const r=tx.objectStore('drafts').get(key);r.onsuccess=()=>{
    ...
    for(const meta of doc.files){
     ...
     q.onsuccess=()=>{const blob=assetBlob(q.result);if(!blob||blob.size!==meta.size){fail(err('DataError','Данные вложения отсутствуют или неполны'));return}files.push({...meta,file:new File([blob],meta.name,{type:meta.type,lastModified:meta.lastModified})});files.sort((a,b)=>doc.files.findIndex(m=>m.id===a.id)-doc.files.findIndex(m=>m.id===b.id))};
    }
   };
  });
 }
...
const signature=s=>JSON.stringify({text:s.text,blocks:s.blocks,composer:s.composer,reply_to:s.reply_to||null,recording:s.recording,selection:s.selection,expanded:s.expanded,files:s.files.map(({file,...m})=>m)});
...
window.DraftVault={assetBlob,uid,DraftStore,Controller,digest,fingerprint,empty,signature,tests};
})();
