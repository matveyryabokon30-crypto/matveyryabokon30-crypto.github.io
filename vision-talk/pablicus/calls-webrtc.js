...
(function(root){'use strict';
 function create({iceServers=[],iceTransportPolicy='all',onSignal,onState,onRemoteStream,onLocalStream}={}){
  let pc=null,local=null,generation=0,queue=Promise.resolve(),candidates=[],acquiring=null,closed=false,caller=false,restarts=0;
  ...
  function accept(signal,{video=false}={}){const g=generation;return serial(async()=>{if(!live(g))return;if(signal.type==='candidate'){if(!pc?.remoteDescription){if(candidates.length>=128)throw Error('candidate-limit');candidates.push(signal.candidate);return;}await pc.addIceCandidate(signal.candidate);return;}if(signal.type!=='description'||!['offer','answer'].includes(signal.description?.type))throw Error('invalid-signal');
   const p=await ensure(video,g);if(!live(g))return;await p.setRemoteDescription(signal.description);if(!live(g))return;for(const c of candidates.splice(0)){await p.addIceCandidate(c);if(!live(g))return;}if(signal.description.type==='offer'){const answer=await p.createAnswer();if(!live(g))return;await p.setLocalDescription(answer);if(live(g))await emit({type:'description',description:p.localDescription.toJSON()});}return local;
  });}
