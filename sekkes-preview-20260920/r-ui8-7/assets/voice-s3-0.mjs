...
import {VoiceIdentityHandshake} from './voice-identity.mjs?v=2026.09.20-ui.8.7';
...
async function startLive(){
 ...
 try{
  ...
  dc.onmessage=e=>{let event;try{event=JSON.parse(e.data)}catch{return}x.finalization.receive(event);if(live!==x)return;x.commands.receive(event);if(live!==x)return;try{if(event.type==='session.updated'&&event.client_event_id===x.commands.id)x.identity.receive({type:'session.started'});else if(event.type!=='session.started')x.identity.receive(event)}catch{}globalThis.window?.SekkesUI?.voiceEvent(event);if(event.type==='session.closed')endLive();if(event.type==='error'&&![x.commands.id,x.commands.id+'_prompt'].includes(event.client_event_id||event.error?.client_event_id||event.error?.event_id))note('Голосовой сервис сообщил об ошибке.')};
