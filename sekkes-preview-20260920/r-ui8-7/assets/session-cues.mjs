...
export class SessionCues {
 ...
 unlock(){
  if(this.primed)return;this.primed=true;const epoch=this.epoch;
  for(const a of Object.values(this.audio))try{
   a.muted=true;
   Promise.resolve(a.play()).then(()=>{if(epoch!==this.epoch)return;a.pause();a.currentTime=0;a.muted=false;}).catch(()=>{a.muted=false;});
  }catch{a.muted=false;}
 }
