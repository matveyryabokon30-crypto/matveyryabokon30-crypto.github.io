...
export function initialize({recorderOptions={},dictationOptions={}}={}){
 ...
 function updateVoice(state){
  ...
  stop.innerHTML=icon(active||waiting||state==='recovery'?'stop':'record');
 ...
 recordButton.innerHTML=icon('record');$('#micButton').innerHTML=icon('mic');$('#sendButton').innerHTML=icon('send');stop.addEventListener('click',()=>{if(['idle','error'].includes(voice))ctx.startVoice();else ctx.runtime()?.toggleMic()},{signal});
