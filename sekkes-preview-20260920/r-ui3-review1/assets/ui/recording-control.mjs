...
export function recordingControl(ctx,panel,mic,signal){
 let url=null,sending=false,requestId=null,pending=null,disposed=false;
 ...
 function render({state,seconds=0,clip,code}){
  panel.replaceChildren();panel.hidden=state==='idle';panel.dataset.state=state;
  mic.disabled=sending||state==='processing';mic.setAttribute('aria-label',state==='recording'?'Остановить запись':state==='permission'?'Отменить запрос микрофона':'Записать голосовое сообщение');mic.innerHTML=icon(state==='recording'||state==='permission'?'close':'mic');
 ...
 const recorder=new ChatRecorder({onState:render});
 async function send(){
  if(sending||!recorder.clip)return;if(ctx.runtime()?.busy)return ctx.notify('Сначала заверши разговор или дождись ответа.');
  sending=true;pending=recorder.clip;requestId ||= crypto.randomUUID();render({state:'sending'});
  try{const response=await ctx.runtime()?.sendVoice({id:requestId,audio:audioBase64(pending.bytes)});if(disposed)return;
  ...
  }catch(e){if(!disposed){render({state:'ready',clip:pending});ctx.notify(e?.code==='DUPLICATE_TURN'?'Сервер уже принял запись. Повторная отправка заблокирована.':'Не удалось получить расшифровку. Запись осталась в чате; можно повторить отправку.')}}
  finally{sending=false;if(!disposed)mic.disabled=false}
 }
 mic.addEventListener('click',()=>{if(recorder.capturing)return recorder.finish();if(ctx.runtime()?.busy)return ctx.notify('Сначала заверши голосовой разговор.');if(recorder.clip)return ctx.notify('Сначала отправь или удали текущую запись.');recorder.start()},{signal});
 ...
 return {get busy(){return recorder.capturing||sending},send,beforeRoute(){if(recorder.capturing)recorder.finish()},reset(){pending=null;requestId=null;revoke();recorder.cancel()},dispose(){disposed=true;revoke();recorder.dispose()}};
}
