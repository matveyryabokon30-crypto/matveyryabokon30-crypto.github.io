...
export function initialize(){
 ...
 let generation=0,current=null,voice='idle',swRegistration=null,recording;
 ...
 const ctx={account:null,messages:null,profileUpdate:null,runtime:()=>window.SekkesS2,
  navigate(id){if(routeId('#'+id)===id)location.hash=id},
  startVoice(){if(recording?.busy)return notify('Сначала заверши запись в чате.');if(voice==='idle')ctx.runtime()?.toggleMic()},
  notify,info(title,text){$('#dialogContent').replaceChildren(el('h2','',title),el('p','',text));$('#dialogContent').firstChild.id='dialogTitle';if(!dialog.open)dialog.showModal()},
 ...
 function appendMessage(role,text,clip){
  ...
  list.append(node);if(atEnd)list.scrollTop=list.scrollHeight;$('#chatAnnouncement').textContent=role==='ai'?'Получен ответ SEKKES':'';
 }
 function updateVoice(state){
  ...
  $('#micButton').hidden=busy;const start=$('#voiceSphere');if(start){start.disabled=busy;start.setAttribute('aria-label',busy?'Голосовой разговор активен':'Начать голосовой разговор')}
  ...
  status.hidden=!['home','chat'].includes(current)&&!busy;
 }
 window.SekkesUI={status(text){status.textContent=text},voice:updateVoice,render:appendMessage,beforeText(){ctx.navigate('chat')},
  get captureBusy(){return Boolean(recording?.busy)},sendRecording(){recording?.send()},
  textBusy(busy){$('#sendButton').disabled=busy;composer.setAttribute('aria-busy',String(busy))},account(user){ctx.account=user;ctx.profileUpdate?.()},
  reset(){recording?.reset();for(const url of attachmentURLs)URL.revokeObjectURL(url);attachmentURLs.clear();pendingMessages.length=0;ctx.messages?.replaceChildren(el('p','empty-state','Диалог пуст.'));ctx.account=null;ctx.profileUpdate?.()},
 ...
 composer.addEventListener('submit',e=>{e.preventDefault();if(recording.busy)return;ctx.runtime()?.sendText()},{signal});
 ...
 recording=recordingControl(ctx,$('#recordingPanel'),$('#micButton'),signal);
 async function route(){
  const id=routeId(location.hash),ticket=++generation,descriptor=sections.find(s=>s.id===id);if(id!=='chat')recording.beforeRoute();
  try{if(!cache.has(id)){const module=await descriptor.loadModule();if(!cache.has(id))cache.set(id,module.create(ctx))}if(ticket!==generation)return;
   ...
   app.dataset.route=id;$('#headerTitle').textContent=id==='chat'?'Чат':'sekkes';composer.hidden=id!=='chat';
 ...
 return {dispose(){lifetime.abort();recording.dispose();dockObserver.disconnect();clearTimeout(notify.timer);for(const url of attachmentURLs)URL.revokeObjectURL(url);for(const screen of cache.values())screen.dispose();cache.clear();window.SekkesUI=null;mounted=false}};
}
