...
let mounted=false;
export function initialize({recorderOptions={}}={}){
 ...
 const ctx={account:null,messages:null,profileUpdate:null,runtime:()=>window.SekkesS2,
  ...
  startVoice(){if(recording?.busy)return notify('Сначала заверши запись в чате.');if(voice==='idle'){ctx.pauseMedia();ctx.runtime()?.toggleMic()}},
 ...
 window.SekkesUI={status(text){status.textContent=text},voice:updateVoice,render:appendMessage,beforeText(){ctx.navigate('chat')},
  ...
  reset(){ctx.pauseMedia();recording?.reset();for(const url of attachmentURLs)URL.revokeObjectURL(url);attachmentURLs.clear();pendingMessages.length=0;ctx.messages?.replaceChildren(el('p','empty-state','Диалог пуст.'));ctx.account=null;ctx.profileUpdate?.()},
 ...
 recording=recordingControl(ctx,$('#recordingPanel'),$('#micButton'),signal,recorderOptions);
 async function route(){
  const id=routeId(location.hash),ticket=++generation,descriptor=sections.find(s=>s.id===id);if(id!=='chat')recording.beforeRoute();if(id!==current)ctx.pauseMedia();
