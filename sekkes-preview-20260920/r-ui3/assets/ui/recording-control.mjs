...
export function recordingControl(ctx,panel,mic,signal,recorderOptions={}){
 ...
 async function send(){
  ...
  }catch(e){if(!disposed&&epoch===generation){render({state:'ready',clip:pending});ctx.notify(e?.code==='DUPLICATE_TURN'?'Сервер уже принял запись. Повторная отправка заблокирована.':'Не удалось получить расшифровку. Запись осталась в чате; можно повторить отправку.')}}
  finally{if(epoch===generation){sending=false;if(!disposed)mic.disabled=false}}
 }
 mic.addEventListener('click',()=>{if(recorder.capturing)return recorder.finish();if(ctx.runtime()?.busy)return ctx.notify('Сначала заверши голосовой разговор.');if(recorder.clip)return ctx.notify('Сначала отправь или удали текущую запись.');ctx.pauseMedia();recorder.start()},{signal});
