...
import {VoiceIdentityHandshake} from './voice-identity.mjs?v=2026.09.20-ui.8.9';
...
function setupDeadline(promise,ms){
 return new Promise((resolve,reject)=>{const timer=setTimeout(()=>reject(new S3Error('LIVE_UNAVAILABLE')),ms);Promise.resolve(promise).then(v=>{clearTimeout(timer);resolve(v)},e=>{clearTimeout(timer);reject(e)});});
}
async function startLive(){
 ...
 const entrance=logged()?sessionCues?.play('start'):null;
 ...
 try{
  ...
  voiceControls('connecting');status('');
  if(unclosedId)await closeSession(unclosedId);
  await preferences.load();check();
  // Finish the cue before iOS switches the audio route to microphone capture.
  await entrance;check();
  x.stream=await setupDeadline(navigator.mediaDevices.getUserMedia({audio:{echoCancellation:true,noiseSuppression:true,autoGainControl:true},video:false}).then(stream=>{if(x.cancelled){for(const t of stream.getTracks())t.stop();}return stream;}),15000);check();
  ...
  const offer=await setupDeadline(pc.createOffer(),5000);check();await setupDeadline(pc.setLocalDescription(offer),5000);check();
  ...
  x.id=answer.id;
  x.identity=new VoiceIdentityHandshake({send:e=>dc.send(JSON.stringify(e)),instruction:answer.startupInstruction,onReady:()=>{if(live===x)status('')},onError:()=>{if(live===x)note('Не удалось подтвердить настройки голоса. Заверши разговор и начни заново.')}});
  x.finalization=prepareFinalization(x);
  x.commands=new VoiceEndCommand({send:event=>dc.send(JSON.stringify(event)),stop:()=>{if(live===x)endLive('Разговор завершён по твоей просьбе.');},onError:()=>{if(live===x)note('Голосовая команда завершения недоступна. Используй кнопку завершения.');}});
  live=x;
  ...
  dc.onopen=()=>{if(live!==x||x.ready)return;x.ready=true;clearTimeout(x.connectTimer);voiceControls('live');status('')};
  dc.onmessage=e=>{let event;try{event=JSON.parse(e.data)}catch{return}x.finalization.receive(event);if(live!==x)return;x.commands.receive(event);if(live!==x)return;try{x.identity.receive(event)}catch{}globalThis.window?.SekkesUI?.voiceEvent(event);if(event.type==='session.closed')endLive();if(event.type==='error'&&![x.commands.id,x.commands.id+'_prompt'].includes(event.client_event_id||event.error?.client_event_id||event.error?.event_id))note('Голосовой сервис сообщил об ошибке.')};
  ...
  await setupDeadline(pc.setRemoteDescription({type:'answer',sdp:answer.sdp}),10000);
  check();if(live!==x)return;
 }catch(e){
  // Stop capture BEFORE any network cleanup or retries.
  x.cancelled=true;sessionCues?.cancel();const wasLive=live===x;if(wasLive)await endLive();else disposeTransport(x);
  ...
  if(!live&&!closing){voiceControls(recoveryNeeded?'recovery':'idle');if(x.cancelled&&!recoveryNeeded)status('');}
 }
}
...
function endLive(message){
 ...
 const x=live;
 if(!x){sessionCues?.cancel();if(startup){disposeTransport(startup);voiceControls('idle');status('');}return Promise.resolve();}
 live=null;x.cancelled=true;
 silenceTransport(x);
 if(x.ready)sessionCues?.play('end');else sessionCues?.cancel();
