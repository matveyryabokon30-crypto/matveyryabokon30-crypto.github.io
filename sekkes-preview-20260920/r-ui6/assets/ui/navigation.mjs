...
export function navigation({root,button,panel,backdrop,app,signal}){
 let open=false,closingTimer=null;
 const paint=()=>{button.setAttribute('aria-expanded',String(open));button.setAttribute('aria-label',open?'Закрыть меню':'Открыть меню');button.innerHTML=icon(open?'close':'menu')};
 function close({restoreFocus=true}={}){
  if(!open)return;open=false;paint();app.inert=false;panel.inert=true;
 ...
 function show(){
  clearTimeout(closingTimer);open=true;panel.hidden=false;panel.inert=false;backdrop.hidden=false;app.inert=true;
  ...
  (panel.querySelector('[aria-current=page]')||panel.querySelector('a')).focus({preventScroll:true});
 }
 ...
 root.addEventListener('keydown',e=>{
  ...
  if(e.key!=='Tab')return;
  const items=[button,...panel.querySelectorAll('a')],i=items.indexOf(root.ownerDocument.activeElement);
 ...
 return {close,dispose(){clearTimeout(closingTimer);app.inert=false;panel.hidden=true;backdrop.hidden=true;root.classList.remove('is-open','is-closing')}};
}
