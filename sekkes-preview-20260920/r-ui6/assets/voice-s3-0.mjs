...
mount();
window.SekkesS2={toggleMic:toggleVoice,sendText,sendVoice,interrupt:()=>endLive(),end:logout,login,voiceSettings:voicePicker,accountAction:()=>logged()?logout():login(),get busy(){return Boolean(live||starting||closing||recovering||textBusy)},get dirty(){return Boolean(live||starting||closing||draft.value.trim()||textHistory.length||loginBusy||textBusy)}};
