...
import {VoiceIdentityHandshake} from './voice-identity.mjs?v=2026.09.20-ui.8.8';
...
async function startLive(){
 ...
 if(logged())sessionCues?.play('start');
 ...
 try{
  ...
  x.id=answer.id;status('Соединяю звук…');
  ...
  dc.onopen=()=>{if(live!==x||x.ready)return;x.ready=true;clearTimeout(x.connectTimer);voiceControls('live');status('Голосовой разговор')};
...
async function toggleVoice(){
 ...
 if(closing){
  if(restartPending)return;restartPending=true;voiceControls('connecting');status('Подключаю голос…');
