...
import {VoiceIdentityHandshake} from './voice-identity.mjs?v=2026.09.20-ui.8.6';
...
function silenceTransport(x){
 ...
 try{x.remoteOutput?.disconnect();x.remoteOutput=null}catch{}
...
async function startLive(){
 ...
 try{
  ...
  pc.ontrack=e=>{if(x.cancelled||live!==x)return;try{x.remoteOutput?.disconnect()}catch{}x.remoteOutput=sessionCues?.attachRemote?.(e.streams[0]);if(x.remoteOutput)return;audio.srcObject=e.streams[0];audio.play().catch(()=>{if(live===x)note('Нажми экран один раз, если звук не включился автоматически.')})};
  ...
  dc.onopen=()=>{if(live!==x||x.ready)return;x.ready=true;sessionCues?.play('start');clearTimeout(x.connectTimer);voiceControls('live');status('Голосовой разговор')};
