...
async function run(useMic=true){
 if(current)return;const x=current={};log.textContent='BEGIN';document.querySelector('#run').disabled=true;
 try{
 cues.begin();if(useMic){const capture=navigator.mediaDevices.getUserMedia({audio:true});x.stream=await capture;if(x.closed){x.stream.getTracks().forEach(t=>t.stop());return}write('MIC: '+x.stream.getAudioTracks()[0].readyState)}else write('MIC: skipped, audio-only test');
...
document.querySelector('#run').onclick=()=>run(true);document.querySelector('#synthetic').onclick=()=>run(false);
