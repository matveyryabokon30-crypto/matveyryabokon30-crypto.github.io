...
import {VoiceIdentityHandshake} from './voice-identity.mjs?v=2026.09.20-ui.8.10';
...
async function startLive(){
 ...
 const markReady=()=>{
  if(live!==x||x.cancelled||x.ready||!x.introDone||!x.channelReady||!x.outputReady)return;
 ...
 try{
  ...
  // Retain capture (not transmission) while the cue plays and connection is prepared.
  for(const t of x.stream.getTracks())t.enabled=false;
  ...
  await entrance;check();if(live!==x)return;for(const t of x.stream.getTracks())t.enabled=true;markReady();
 }catch(e){
  x.cancelled=true;voiceTrace('error',e.code||x.stage+'_'+e.name);sessionCues?.cancel();sessionCues?.resetRoute?.();
  ...
  if(!live&&!closing){voiceControls(recoveryNeeded?'recovery':'idle');status('');}
 }
}
...
function endLive(message){
 ...
 const x=live;
 if(!x){sessionCues?.cancel();sessionCues?.resetRoute?.();if(startup){disposeTransport(startup);voiceControls('idle');status('');}return Promise.resolve();}
