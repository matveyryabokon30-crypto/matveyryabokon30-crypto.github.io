import {normalizePreference} from '../../vnext-foundation/foundation.mjs';

// A voice preference is user-editable data, never an authorization claim.
export class VoicePreferences {
  constructor({profile, enabledIds, storage, onChange=()=>{}}) {
    Object.assign(this,{profile,enabledIds,storage,onChange});
    this.selected='echo';this.epoch=0;this.busy=false;this.loaded=false;
  }
  async load() {
    ...
    const {id}=normalizePreference(saved,this.enabledIds);
  ...
  reset(){++this.epoch;this.busy=false;this.loaded=false;this.selected='echo';this.onChange('echo');}
}
