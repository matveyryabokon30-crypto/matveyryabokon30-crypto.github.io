...
(() => {
  'use strict';
  const BUILD = '2026.09.19-p23-stage.6';
  ...
  function openDialog(name) {
    ...
    if (name === 'profile') {
      $('#s3ProfileVoice').addEventListener('click',()=>{if(window.SekkesS2?.voiceSettings)window.SekkesS2.voiceSettings();else notify('Каталог голосов загружается.');});
  ...
  async function checkUpdate() {
    ...
    try {
      ...
      if (typeof info.version === 'string' && /^[0-9A-Za-z._-]{1,64}$/.test(info.version) && info.app === 'sekkes-p23-stage' && info.version !== BUILD) {
