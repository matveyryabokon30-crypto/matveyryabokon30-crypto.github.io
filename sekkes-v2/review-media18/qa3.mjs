...
try{
 ...
 const oi=owner.node.querySelector('input[type=file]');const odt=new DataTransfer();odt.items.add(new File([bytes],'UUID-owner.png',{type:'image/png'}));oi.files=odt.files;oi.dispatchEvent(new Event('change'));for(let i=0;i<30&&!owner.node.querySelector('.owner-attachments img');i++)await pause();check(!!owner.node.querySelector('.owner-attachments img'),'admin composer thumbnail');const os=owner.node.querySelector('.owner-send').getBoundingClientRect();check(os.width===34&&os.height===34,'admin round send control');
