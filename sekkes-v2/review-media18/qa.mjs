...
try{
 ...
 const owner=create({runtime:()=>state,account:{id:'synthetic'},bindMessage:()=>{},notify:console.log});document.querySelector('#ownerHost').append(owner.node);await owner.refresh();await pause();await pause();check(!!owner.node.querySelector('.owner-user img'),'admin photo auto preview');check(!owner.node.querySelector('.owner-user').textContent.includes('UUID'),'admin hides photo filename');
