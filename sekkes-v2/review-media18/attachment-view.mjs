import {el} from '../assets/ui/components.mjs';
...
export function mediaView(m,url,{compact=false}={}){
 ...
 if((m.mime||'').startsWith('image/')){const img=el('img');img.src=url;img.alt='Фото';img.decoding='async';wrap.append(img);}
...
export function mediaLoader(){
 const jobs=new Map();const observer=globalThis.IntersectionObserver?new IntersectionObserver(items=>{for(const i of items)if(i.isIntersecting){observer.unobserve(i.target);const run=jobs.get(i.target);jobs.delete(i.target);void run?.();}},{rootMargin:'160px'}):null;
 return {mount(m,getURL){const wrap=el('div','attachment-view attachment-loading');wrap.setAttribute('aria-label',m.mime?.startsWith('video/')?'Видео':'Фото');let busy=false;
 const run=async()=>{if(busy)return;busy=true;wrap.replaceChildren();wrap.classList.add('attachment-loading');try{const url=await getURL();if(!url)return;wrap.replaceChildren(mediaView(m,url));wrap.classList.remove('attachment-loading');}catch{wrap.classList.remove('attachment-loading');const retry=el('button','attachment-retry','Загрузить ещё раз');retry.type='button';retry.onclick=run;wrap.replaceChildren(retry);}finally{busy=false}};
 if(observer){jobs.set(wrap,run);observer.observe(wrap);}else queueMicrotask(run);return wrap;},reset(){observer?.disconnect();jobs.clear();}};
}
