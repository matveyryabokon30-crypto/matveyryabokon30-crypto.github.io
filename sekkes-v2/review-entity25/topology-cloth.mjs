...
export const THREAD_GAP=1.18, CLOTH_MARGIN=320;
...
export class ClothField{
 ...
 resize(width,height){
  ...
  this.driftX=0;this.driftY=0;this.breath=1;this.angle=-.72;this.cos=Math.cos(this.angle);this.sin=Math.sin(this.angle);
  this.strands=[];this.bands=Array.from({length:12},()=>[]);this.longitudinal=new Float32Array(this.rows);
  ...
  for(let x=-this.extent,i=0;x<=this.extent+THREAD_GAP;i++){
   ...
   const strand={x,band:Math.floor(light*12)};this.strands.push(strand);this.bands[strand.band].push(strand);x+=THREAD_GAP*(.84+.32*hash(i));
  }
 }
 update(field,seconds=0){
  this.angle=-.72+seconds*.018+.78*Math.sin(seconds*.061)+.28*Math.sin(seconds*.137);
  this.driftX=46*Math.sin(seconds*.103)+18*Math.sin(seconds*.223);
  this.driftY=42*Math.cos(seconds*.089)+16*Math.sin(seconds*.191);
  this.breath=1+.04*Math.sin(seconds*.17);
  ...
  for(let row=0;row<rows;row++){
   ...
   const fold=92*Math.sin(v/190+seconds*.16+.65*Math.sin(seconds*.079))+42*Math.sin(v/310-seconds*.123)+18*Math.sin(v/115+seconds*.207);
   this.longitudinal[row]=v+34*Math.sin(v/290+seconds*.183)+18*Math.sin(v/175-seconds*.137);
   for(let col=0;col<cols;col++){
    ...
    values[row*cols+col]=fold+165*(x*cos+y*sin)/(Math.hypot(x,y)+.035);
   }
 ...
 point(x,row,out){
  const u=x+this.displacement(x,row),v=this.longitudinal[row];
  out[0]=this.width/2+this.driftX+(u*this.cos-v*this.sin)*this.breath;out[1]=this.height/2+this.driftY+(u*this.sin+v*this.cos)*this.breath;return out;
 }
 draw(cx,color){
  cx.globalCompositeOperation='source-over';cx.globalAlpha=1;cx.fillStyle='#002222';cx.fillRect(0,0,this.width,this.height);
  cx.save();cx.translate(this.width/2+this.driftX,this.height/2+this.driftY);cx.rotate(this.angle);cx.scale(this.breath,this.breath);cx.lineWidth=.95;
  // Cubic B-spline in both axes: no corners in the flow or kinks between fibres.
  const y=row=>this.longitudinal[row];
  for(let band=0;band<12;band++){
   cx.beginPath();
   for(const strand of this.bands[band]){
    const x=strand.x;let a=x+this.displacement(x,0),b=x+this.displacement(x,1),c=x+this.displacement(x,2);
    cx.moveTo((a+4*b+c)/6,(y(0)+4*y(1)+y(2))/6);
    for(let row=1;row<this.rows-2;row++){
     const d=x+this.displacement(x,row+2);
     cx.bezierCurveTo((2*b+c)/3,(2*y(row)+y(row+1))/3,(b+2*c)/3,(y(row)+2*y(row+1))/3,(b+4*c+d)/6,(y(row)+4*y(row+1)+y(row+2))/6);a=b;b=c;c=d;
    }
   }
   cx.strokeStyle=`rgba(${color.join(',')},${.49+band*.032})`;cx.stroke();
  }
  cx.restore();
 }
}
