...
initialize();const pause=()=>new Promise(r=>setTimeout(r,250));const wait=async fn=>{for(let i=0;i<60&&!fn();i++)await pause();if(!fn())throw Error('Timeout')};
...
document.querySelector('#runCheck').onclick=async()=>{const checks=[];const check=(name,value)=>{checks.push([name,!!value]);results.textContent=checks.map(([n,v])=>(v?'PASS ':'FAIL ')+n).join('\n')};try{
 ...
 document.querySelector('.chat-bottom').click();await pause();media.style.height='450px';await pause();check('single jump survives media resize gap='+gap()+' follow='+list.timelineController.following,gap()<3);
