...
export function mountLivingIcons(doc=document){
 const win=doc.defaultView,records=new Map(),dormant=new WeakMap(),samples=new Map(),media=win.matchMedia('(prefers-reduced-motion: reduce)');
 ...
 const interacting=()=>doc.documentElement.dataset.interacting==='true';
 ...
 function paint(record,still=false){
  ...
  cx.globalAlpha=1;if(record.canvas.hidden)record.canvas.hidden=false;if(!record.node.classList.contains('is-live'))record.node.classList.add('is-live');
 }
 ...
 function schedule(){if(disposed||doc.hidden||reduced()||interacting()||raf||![...records.values()].some(eligible))return;raf=win.requestAnimationFrame(tick);}
 function tick(now){
  raf=0;if(disposed||doc.hidden||reduced()||interacting()){previous=null;return;}
  ...
  if(now-lastPaint>=1000/24){
   lastPaint=now;for(const record of records.values())if(eligible(record)){
    try{paint(record)}catch{record.node.classList.remove('is-live');record.canvas.hidden=true;record.failed=true;}
   }
  }
  schedule();
 }
 function register(node){
  ...
  try{
   ...
   const name=node.dataset.livingIcon,key=path.getAttribute('d'),cached=dormant.get(node);
   if(cached&&cached.canvas===canvas&&cached.name===name&&cached.key===key){records.set(node,cached);dormant.delete(node);intersection?.observe(node);resize?.observe(node);return;}
   let anchors=samples.get(key);
   ...
   const record={node,canvas,cx,name,key,anchors,points:new Float32Array(anchors.length*2),phase:serial++*.67+iconSeed(name)*9,visible:node.getClientRects().length>0,pressedAt:-100,impulse:0};
   records.set(node,record);intersection?.observe(node);resize?.observe(node);
  }catch{/* Keep the recognizable vector fallback if Canvas/path APIs fail. */}
 }
 function sync(){
  syncFrame=0;if(disposed)return;
  if(interacting()){pending=true;stop();return;}pending=false;
  for(const [node] of records)if(!node.isConnected){intersection?.unobserve(node);resize?.unobserve(node);dormant.set(node,records.get(node));records.delete(node);}
 ...
 function queue(){if(disposed)return;pending=true;if(syncFrame||interacting())return;syncFrame=win.requestAnimationFrame(sync);}
 const intersection=win.IntersectionObserver?new win.IntersectionObserver(entries=>{for(const entry of entries){const r=records.get(entry.target);if(r){r.visible=entry.isIntersecting;if(r.visible&&(!r.laidOut||!r.width))queue();}}if(![...records.values()].some(eligible))stop();else schedule();}):null;
 const resize=win.ResizeObserver?new win.ResizeObserver(()=>queue()):null;
 // Ignore unrelated DOM churn and idempotent aria writes. A scroll pause is
 // not Reduced Motion: keep the exact canvas frame instead of reassembling it.
 const hasIcon=node=>node?.matches?.(selector)||Boolean(node?.querySelector?.(selector));
 const observer=new win.MutationObserver(changes=>{
  let changed=false,motionChanged=false;
  for(const change of changes){
   if(change.type==='attributes'&&change.oldValue===change.target.getAttribute(change.attributeName))continue;
   if(change.target===doc.documentElement){if(change.attributeName==='data-interacting')motionChanged=true;else changed=true;continue;}
   if(change.type==='childList')changed||=[...change.addedNodes,...change.removedNodes].some(hasIcon);
   else if(!change.target.matches?.('.living-icon-canvas'))changed||=hasIcon(change.target);
  }
  if(changed)queue();
  if(motionChanged){if(interacting())stop();else if(pending)queue();else schedule();}
 });
 observer.observe(doc.body,{childList:true,subtree:true,attributes:true,attributeOldValue:true,attributeFilter:['hidden','open','disabled','aria-pressed','aria-expanded']});
 observer.observe(doc.documentElement,{attributes:true,attributeOldValue:true,attributeFilter:['data-motion','data-theme','data-interacting']});
 function visibility(){if(doc.hidden)stop();else{previous=null;queue();}}
 function react(event){
  if(event.type==='keydown'&&!['Enter',' '].includes(event.key))return;
  const control=event.target.closest?.('button,a');if(!control||control.disabled||reduced()||interacting())return;
