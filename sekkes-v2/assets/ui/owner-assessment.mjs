...
export function encodeAssessment(value){
 ...
 if(!value.question?.trim()||!value.expected?.trim()||(value.verdict!=='pending'&&!value.actual?.trim()))throw Error('Заполни вопрос и ожидаемый результат. Для оценки «Пройдено» или «Не пройдено» также нужен полученный ответ.');
...
export function assessmentEditor({value={},versions=[],onDirty=()=>{},onSave}){
 ...
 for(const [key,label,multiline]of definitions){const wrap=el('label','owner-assessment-field'),input=el(multiline?'textarea':'input','owner-input');input.setAttribute('aria-label',label);input.maxLength=key==='title'?160:25000;if(multiline)input.rows=key==='notes'?3:5;input.value=value[key]||'';input.readOnly=Boolean(value.source&&['question','actual'].includes(key));input.required=!['notes','actual'].includes(key);input.oninput=onDirty;fields[key]=input;wrap.append(el('span','owner-note',label),input);form.append(wrap);}
 ...
 form.append(el('p','owner-note','Для новой проверки достаточно вопроса и ожидаемого результата; оставь оценку «Нужна проверка». Полученный ответ можно заполнить позже. Сохранение не запускает модель.'),verdict,error,save);
