...
export function create(ctx){
 ...
 function personal(){header('Личные настройки');if(ctx.account?.email)body.append(el('p','profile-note','Почта аккаунта: '+ctx.account.email));const group=el('div','settings-group');group.append(row({title:'Редактировать анкету и аватар',iconName:'edit',action:edit}),row({title:'Войти с электронной почтой',iconName:'mail',action:()=>ctx.runtime()?.emailLogin()}),row({title:'Face ID / ключ доступа',iconName:'shield',action:()=>ctx.runtime()?.faceIdSettings()}),row({title:'Голос помощника',iconName:'sound',action:()=>ctx.runtime()?.voiceSettings()}),row({title:'Оформление приложения',iconName:'settings',action:()=>ctx.navigate('settings')}),row({title:ctx.account?'Выйти из аккаунта':'Войти в AI Marius',iconName:'profile',action:()=>ctx.runtime()?.accountAction()}));body.append(group);}
