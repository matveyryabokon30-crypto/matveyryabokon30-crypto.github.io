...
const names={text:'Текстовый разговор',chat:'Административный разговор',training:'Учебные задания',evaluation:'Проверки',memory_test:'Проверки памяти',context_test:'Проверки контекста',live:'Живой голос',live_backend:'Рассуждения в голосе',transcription:'Распознавание записи',speech:'Озвучивание'};
...
export function billingPanel({request}){
 ...
 function render(d){
  ...
  report.append(line('Расчётная сумма известных расходов',money(s.estimated_usd)),note('Это оценка по измеренному использованию и тарифам. Она не равна подтверждённому списанию.'),line('Действия / обращения к моделям',`${s.actions??0} / ${s.calls??0}`),line('Записи с неполным учётом',s.incomplete??0));
  const imp=d.import;
