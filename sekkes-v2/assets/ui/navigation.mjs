...
export function navigation({root,button,panel,backdrop,app,signal}){
 ...
 function close({restoreFocus=true}={}){
  if(!open){app.removeAttribute('inert');panel.inert=true;panel.hidden=true;backdrop.hidden=true;root.classList.remove('is-open','is-opening','is-closing');return;}open=false;clearTimeout(openingTimer);root.classList.remove('is-opening');paint();app.removeAttribute('inert');panel.inert=true;
 ...
 function show(){
  clearTimeout(closingTimer);clearTimeout(openingTimer);root.classList.add('is-opening');openingTimer=setTimeout(()=>root.classList.remove('is-opening'),220);open=true;panel.hidden=false;panel.inert=false;backdrop.hidden=false;app.setAttribute('inert','');
 ...
 return {close,dispose(){clearTimeout(closingTimer);clearTimeout(openingTimer);app.removeAttribute('inert');panel.hidden=true;backdrop.hidden=true;root.classList.remove('is-open','is-closing','is-opening')}};
}
