...
export function storyEditor({host,dialog,save,current=()=>true,onPending=()=>{},errorText=e=>e.message}){
 ...
 const trash=el('div','se-trash');trash.setAttribute('aria-hidden','true');trash.innerHTML=icon('trash');trash.append(el('span','','Удалить'));trash.hidden=true;work.append(trash);
 ...
 function bindGestures(){
  ...
  const end=e=>{if(!pointers.has(e.pointerId))return;const p={x:e.clientX,y:e.clientY};pointers.delete(e.pointerId);if(pointers.size){base();return;}const layer=selected&&surface.state.layers.find(l=>l.id===selected),tr=trash.getBoundingClientRect(),drop=!trash.hidden&&p.x>=tr.left&&p.x<=tr.right&&p.y>=tr.top&&p.y<=tr.bottom;if(snapshot()!==before)remember(before);before='';root.classList.remove('se-layer-dragging');trash.hidden=true;finishGesture();if(drop&&layer){removeLayer(layer);return;}schedule();};
