/** Browser API client; never accepts arbitrary endpoints or an OpenAI key. Auth token stays in memory. */
...
export class S2Api {
  ...
  async login(email,password){
    ...
    if(!r.ok)throw new S2Error(r.status===429?'LOGIN_RATE_LIMIT':'LOGIN_FAILED');const d=await r.json();if(!d.access_token)throw new S2Error('LOGIN_FAILED');if(epoch!==this.authEpoch)throw new S2Error('AUTH_CANCELLED');this.token=d.access_token;this.expiresAt=Date.now()+(Number(d.expires_in)||3600)*1000;
    // No automatic refresh token persistence and no user metadata in localStorage.
  }
  ...
  async request(path,{method='GET',body,signal}={}){
    ...
    const boundSignal=signal?AbortSignal.any([signal,AbortSignal.timeout(path==='turn'?85000:12000)]):AbortSignal.timeout(path==='turn'?85000:12000);
    const r=await this.transport(this.base+'/'+path,{method,cache:'no-store',redirect:'error',signal:boundSignal,headers:{apikey:this.config.publishableKey,...(this.token?{Authorization:'Bearer '+this.token}:{}),...(body?{'Content-Type':'application/json'}:{})},body:body?JSON.stringify(body):undefined});
    let d;try{d=await r.json();}catch{throw new S2Error('SERVICE_UNAVAILABLE');}if(!r.ok){if(r.status===401)this.logout();throw new S2Error(d.code||'SERVICE_UNAVAILABLE');}return d;
  }
}
