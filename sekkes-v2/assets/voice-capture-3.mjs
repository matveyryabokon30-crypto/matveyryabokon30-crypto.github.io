/** Local capture. No network inside this module. PCM WAV; maximum 45 seconds. */
export function encodeWav(chunks,inputRate) {
  ...
  // Average source samples per destination sample; controlled mono input.
...
export class VoiceCapture {
  ...
  async start(){
    ...
    try{
      ...
      if(seq!==this.seq||document.hidden){stream.getTracks().forEach(t=>t.stop());return;}
      const C=window.AudioContext||window.webkitAudioContext;
      this.context=new C();this.stream=stream;
      await this.context.audioWorklet.addModule(new URL('./voice-worklet-3.mjs',import.meta.url));
      if(seq!==this.seq||document.hidden){this.cancel();return;}
