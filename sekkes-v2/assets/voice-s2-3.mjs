...
function setState(s){state=s;document.body.classList.toggle('mic-active',s==='capturing');const names={disabled:api?'Войди в SEKKES для разговора':'Подключение к серверу',signedout:'Войди в SEKKES для разговора',idle:'Готов к разговору',permission:'Ожидаем разрешение микрофона',capturing:'Слушаю · до 45 секунд',sending:'Готовлю ответ',speaking:'Говорит AI',paused:'Пауза',error:'Не удалось завершить запрос'};status(names[s]||s);const enter=$('#s2LoginButton');if(enter){enter.hidden=Boolean(api?.token);enter.disabled=!api;}const replay=$('#s2Playback');if(replay)replay.disabled=!lastAudio||busy();$('#micButton').setAttribute('aria-label',s==='capturing'?'Отправить голосовую фразу':s==='sending'||s==='speaking'?'Прервать ответ':'Говорить с AI');$('#micTestLink').textContent=s==='capturing'?'Отправить фразу':busy()?'Остановить':'Начать голосовой разговор';const end=$('#s2End');if(end)end.disabled=!api?.token&&!history.length;}
