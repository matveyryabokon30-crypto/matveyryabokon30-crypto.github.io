...
async function startLive(){
 ...
 try{
  ...
  dc.onmessage=e=>{let event;try{event=JSON.parse(e.data)}catch{return}x.finalization.receive(event);if(live!==x)return;x.identity.receive(event);if(event.type==='session.closed')endLive();if(event.type==='error')note('Голосовой сервис сообщил об ошибке.')};
...
function mount(){const tools=document.createElement('div');tools.className='s3-tools';tools.innerHTML='<button id="s3LoginButton" class="s3-login">Войти в SEKKES</button>';$('#aiFull').append(tools);$('#s3LoginButton').onclick=()=>logged()?logout():login();}
mount();
window.SekkesS2={toggleMic:toggleVoice,sendText,interrupt:()=>endLive(),end:logout,login,voiceSettings:voicePicker,get busy(){return Boolean(live||starting||closing||textBusy)},get dirty(){return Boolean(live||starting||closing||draft.value.trim()||textHistory.length||loginBusy||textBusy)}};
