import {prepareFinalization} from './voice-finalize.mjs?v=2026.09.20-s3.25';
import {VoiceIdentityHandshake} from './voice-identity.mjs?v=2026.09.20-s3.25';
import {VoicePreferences,supabaseVoiceProfile} from './preferences.mjs?v=2026.09.20-s3.25';
import {VoicePicker} from './voice-picker.mjs?v=2026.09.20-s3.25';
import{S3Api,S3Error}from'./s3-api.mjs?v=2026.09.20-s3.25';
...
function endLive(message){if(!live)return closing;const x=live;live=null;x.identity?.dispose();clearTimeout(x.timer);clearTimeout(x.connectTimer);
 const serverClose=x.id?closeSession(x.id):Promise.resolve();
 const transportClose=x.finalization?.finish()||Promise.resolve(false);
 const cleanup=Promise.allSettled([serverClose,transportClose]).then(results=>{
  x.stream?.getTracks().forEach(t=>t.stop());try{x.pc?.close()}catch{}try{x.audio.srcObject=null}catch{}
  if(results[0].status==='rejected')throw results[0].reason;
 });
 closing=cleanup;cleanup.then(()=>{if(closing===cleanup)closing=null},()=>{if(closing===cleanup)closing=null});
 document.body.classList.remove('s3-live');$('#s3EndLive').hidden=true;$('#micButton').setAttribute('aria-label','Начать голосовой разговор');status(logged()?'Можно говорить':'Войди в SEKKES');if(message)note(message);return cleanup;
}
