...
(() => {
  ...
  $('#micTestLink').addEventListener('click', () => { if (mic || micPending) stopMic(); else openDialog('mic'); });
  ...
  const dialogViews = {
    ...
    profile: () => `<div class="kicker">ТВОЁ ПРОСТРАНСТВО</div><h2 id="dialogTitle">Настройки</h2><div class="settings-row"><span>Палитра</span><div class="palette"><button class="luna" data-theme="luna" aria-label="Палитра Luna" aria-pressed="false"></button><button class="moon" data-theme="moon" aria-label="Палитра Moon" aria-pressed="false"></button><button class="crimson" data-theme="crimson" aria-label="Палитра Crimson" aria-pressed="false"></button></div></div><label class="settings-row"><span>Меньше движения</span><input id="motionSetting" type="checkbox"></label><div class="settings-row"><span>Личная память AI</span><span class="hint">Не подключена</span></div><p>Сейчас здесь проверяется новый интерфейс. Учётная запись, личная память и AI не подключены. Сохраняются только выбранная палитра и настройка движения.</p><p class="hint">Сборка ${BUILD}. Дизайн-кандидат S1.<br>Без трекеров, записи звука и отправки текста.</p>`,
  ...
  function canReload() { return !document.hidden && route === 'home' && !dialog.open && !draft.value.trim() && !mic && !micPending && !pointer; }
  ...
  document.addEventListener('visibilitychange', () => { if (document.hidden) stopMic(); else { geometry(); checkUpdate(); } });
