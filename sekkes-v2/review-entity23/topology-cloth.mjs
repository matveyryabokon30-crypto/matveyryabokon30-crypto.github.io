...
export const THREAD_GAP=1.4, CLOTH_MARGIN=240;
...
export class ClothField{
 ...
 update(field,seconds=0){
  this.angle=-.72+seconds*.012+.42*Math.sin(seconds*.035)+.14*Math.sin(seconds*.087);
  this.driftX=28*Math.sin(seconds*.063)+12*Math.sin(seconds*.137);
  this.driftY=24*Math.cos(seconds*.051)+10*Math.sin(seconds*.119);
  this.breath=1+.025*Math.sin(seconds*.11);
  ...
  for(let row=0;row<rows;row++){
   ...
   const fold=52*Math.sin(v/210+seconds*.075+.38*Math.sin(seconds*.053))+24*Math.sin(v/340-seconds*.061);
   for(let col=0;col<cols;col++){
    ...
    values[row*cols+col]=fold+145*(x*cos+y*sin)/(Math.hypot(x,y)+.035);
   }
 ...
 draw(cx,color){
  ...
  for(let band=0;band<12;band++){
   ...
   cx.strokeStyle=`rgba(${color.join(',')},${.46+band*.031})`;cx.stroke();
  }
  cx.restore();
 }
}
